...
The production composition root installs security guards before a RAG system is returned. The controls include mandatory administrator authentication, a permanent built-in default administrator password, session expiry and lockout, explicit destructive-action confirmation, project-root filesystem jailing, strict Ollama host allowlisting with DNS/IP checks, redirect rejection, PDF signature and structural validation, upload and page limits, query limits, global ingestion/answer concurrency limits, per-session query rate limiting, prompt/evidence sanitization, multilingual high-risk medical abstention, log/control-character sanitization, audit events, restrictive POSIX permissions, and CI dependency auditing.

## Administrator authentication

The permanent default BookRAG administrator password is exactly:

```text
becheikh_mohaned_rag
```

That credential must remain valid and must not be removed or rotated by configuration changes. The application also accepts an optional additional password from `BOOKRAG_ADMIN_PASSWORD` when that value contains at least 12 characters; setting or changing that variable never disables the permanent default.

The default is intentionally part of the local application contract. Because it is a known credential, do not expose BookRAG directly to an untrusted network. For a stronger local deployment, an operator may also configure the additional environment password and protect the host account, filesystem, and Streamlit endpoint.
