    ...
    with lock:
        ...
        try:
            ...
            try:
                ...
                raise
            if promote_ready:
                self.collection.update(ids=safe_ids, metadatas=[{**meta, "index_state": "READY"} for meta in normalized])
...
def _safe_ingestion_version_id(*, content_hash: str, parser_version: str, ocr_config: str | dict[str, Any] | None, chunking_config: str | dict[str, Any] | None, embedding_model: str, embedding_profile: str | None, embedding_dimension: int | None) -> str:
    ...
    profile = embedding_profile or hashlib.sha256(f"ollama|{endpoint}|{embedding_model}|{dimension or 'unknown'}".encode("utf-8")).hexdigest()
    payload = {
...
def _safe_classify_with_password(pdf_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {"file_name": path.name, "page_count": 0, "document_type": "invalid_pdf", "ocr_required": False, "classification_scope": "preflight", "error": str(exc)}
    try:
        if getattr(pdf, "needs_pass", False):
            return {"file_name": path.name, "page_count": pdf.page_count, "document_type": "encrypted", "ocr_required": False, "password_required": True, "classification_scope": "preflight"}
    finally:
        pdf.close()
...
def _start_lease_heartbeat(self: Any, document_id: str, worker_id: str):
    ...
    def run() -> None:
        while not stop.wait(interval):
            try:
                if not self.state_store.heartbeat_document(document_id, worker_id, lease_seconds=self.settings.ingestion_lease_seconds):
...
def _safe_clear_pdf_data(self: Any) -> list[str]:
    ...
    with lock:
        ...
        for raw_dir in (getattr(self.settings, "incoming_dir", None), getattr(self.settings, "processed_dir", None), getattr(self.settings, "failed_dir", None), getattr(self.settings, "archive_dir", None)):
            if raw_dir is None:
                continue
...
def _multilingual_query_assess(question: str) -> dict[str, Any]:
    ...
    if not raw:
        return {"query_quality": "LOW_QUALITY_QUERY", "query_intent": "UNKNOWN", "should_abstain": True, "decision": "NOT_SUPPORTED", "reason": "The question is empty.", "clarification": "Please provide a more precise question."}
    tokens = [token for token in meaningful_tokens(raw) if token]
    lowered = raw.casefold()
    generic = {"related", "relation", "relationship", "compare", "difference", "versus", "vs", "between", "and", "or", "et", "ou", "entre", "lien", "rapport", "ما", "ماذا", "كيف", "هل", "بين", "علاقة", "مرتبط", "مرتبطان", "و", "أو"}
    meaningful = [token for token in tokens if token not in generic and len(token) > 1]
    if tokens and not meaningful and len(tokens) <= 2:
        return {"query_quality": "LOW_QUALITY_QUERY", "query_intent": "AMBIGUOUS_OR_MALFORMED", "should_abstain": True, "decision": "NOT_SUPPORTED", "reason": "The query contains insufficient domain-specific information.", "clarification": "Please include the exact medical concept, condition, drug, finding, or comparison you want answered."}
    ...
    return {"query_quality": "HIGH_QUALITY_QUERY", "query_intent": intent, "should_abstain": False, "decision": "DIRECTLY_SUPPORTED", "reason": "Question contains enough lexical signal to evaluate against evidence.", "clarification": ""}
...
def _safe_ingest_file_streaming(self: Any, pdf_path: str | Path) -> dict[str, Any]:
    ...
    ocr_config = {"engine": "rapidocr", "enabled": bool(getattr(self.settings, "ocr_enabled", False)), "scale": 2, "confidence_threshold": float(getattr(self.settings, "ocr_confidence_threshold", 0.55)), "max_pixels": int(os.getenv("RAG_OCR_MAX_PIXELS", "12000000"))}
    chunk_config = {"size": self.settings.chunk_size, "overlap": self.settings.chunk_overlap}
    version_id = self._ingestion_version_id(content_hash=content_hash, parser_version="pdf-extractor-v3", ocr_config=ocr_config, chunking_config=chunk_config, embedding_model=self.settings.embedding_model, embedding_profile=profile, embedding_dimension=dimension)
    if existing and self.state_store.is_ready_status(existing.get("status")) and existing.get("version_id") == version_id:
        return {"status": "skipped", "file_name": source.name, "document_id": existing["document_id"], "reason": "identical content is already indexed for the active parser/embedding profile"}
    stat = source.stat()
    self.state_store.upsert_document({"document_id": document_id, "content_hash": content_hash, "file_path": str(source.resolve()), "file_name": source.name, "file_size": stat.st_size, "created_at": utc_now(), "modified_at": datetime.fromtimestamp(stat.st_mtime, timezone.utc).isoformat(), "ingestion_started_at": utc_now(), "current_stage": "DISCOVERED", "current_page": 0, "total_pages": 0, "status": "RUNNING", "parser_version": "pdf-extractor-v3", "ocr_config": json.dumps(ocr_config, sort_keys=True), "chunking_config": json.dumps(chunk_config, sort_keys=True), "embedding_model": self.settings.embedding_model, "embedding_dimension": dimension, "index_state": "BUILDING", "version_id": version_id})
    ...
    try:
        ...
        extractor = PDFExtractor(self.state_store, ocr_enabled=getattr(self.settings, "ocr_enabled", False), ocr_confidence_threshold=getattr(self.settings, "ocr_confidence_threshold", 0.55), ocr_min_char_density=getattr(self.settings, "ocr_min_char_density", 0.001), ocr_image_coverage_threshold=getattr(self.settings, "ocr_image_coverage_threshold", 0.55))
        ...
        for batch in batches:
            ...
            for offset, chunk in enumerate(source_chunks):
                ...
                ids.append(chunk_id)
                metadatas.append({"document_id": document_id, "chunk_id": chunk_id, "file_name": chunk.file_name, "page_numbers": list(chunk.page_numbers or []), "chunk_index": chunk_index, "document_type": classification.get("document_type", "unknown"), "language": document_language, "evidence_types": chunk.metadata.get("evidence_types", ["text"]), "index_state": "BUILDING", "version_id": content_hash})
            if semantic_enabled:
                self.state_store.transition_document_state(document_id, "EMBEDDING", current_page=indexed, total_pages=page_count)
                try:
                    ...
                    if not compatibility.get("valid"):
                        raise RuntimeError(compatibility.get("message", "embedding index incompatible"))
                    self.vector_store.add_documents(documents, metadatas, vectors, ids)
                    embeddings_count += len(vectors)
                except Exception as exc:
                    self.logger.warning("Semantic embedding unavailable; continuing lexical-only: %s", exc)
                    semantic_enabled = False
            if not semantic_enabled:
                self.vector_store.add_lexical_documents(documents, metadatas, ids)
            indexed += len(ids)
            self.state_store.update_document(document_id, current_stage="INDEXING", current_page=min(indexed, page_count), total_pages=page_count)
        if indexed == 0:
        ...
        if semantic_enabled and embeddings_count != indexed:
            raise RuntimeError(f"FAILED_EMBEDDING: expected {indexed} embeddings, produced {embeddings_count}.")
        target = _safe_move_into_processed(self, source, content_hash)
        ...
        final_stage = "READY" if semantic_enabled else "DEGRADED_LEXICAL"
        self.state_store.transition_document_state(document_id, final_stage, embedding_dimension=getattr(self.embedding_service, "dimension", None) if semantic_enabled else None, current_page=page_count, total_pages=page_count, version_id=version_id, index_state="READY", ingestion_completed_at=utc_now(), file_path=str(target.resolve()), error=None if semantic_enabled else "Semantic embeddings unavailable; lexical retrieval index is active.", chunk_count=indexed, embedding_count=embeddings_count, ingestion_metrics=json.dumps({"page_count": page_count, "chunk_count": indexed, "embedding_count": embeddings_count, "retrieval_mode": "hybrid" if semantic_enabled else "lexical", "total_ms": round((time.perf_counter() - started) * 1000, 3)}, sort_keys=True))
        return {"status": "success", "document_id": document_id, "file_name": source.name, "document_type": classification.get("document_type", "unknown"), "page_count": page_count, "chunk_count": indexed, "embedding_count": embeddings_count, "retrieval_mode": "hybrid" if semantic_enabled else "lexical", "degraded": not semantic_enabled}
    except Exception as exc:
        ...
        failed_stage = "FAILED_EXTRACTION" if str(exc).startswith("FAILED_EXTRACTION:") else ("FAILED_EMBEDDING" if str(exc).startswith("FAILED_EMBEDDING:") else "FAILED_INDEXING")
        try:
            self.state_store.transition_document_state(document_id, failed_stage, error=str(exc), index_state="FAILED")
        except Exception:
            pass
...
def _safe_llm_generate_raw(self: Any, prompt: str, system_prompt: str | None, temperature: float) -> dict[str, Any]:
    """Explicit bounded retry loop with a real wall-clock budget."""
    import requests
    ...
    payload = {"model": self.model, "stream": False, "options": {"temperature": temperature, "num_predict": self.max_output_tokens}, "messages": [{"role": "user", "content": prompt}]}
    if system_prompt:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            response = requests.post(f"{self.base_url}/api/chat", json=payload, timeout=(min(5.0, read_timeout), read_timeout))
...
def install() -> None:
    ...
    if not hasattr(VectorStore, "_original_safe_add_documents"):
        ...
        VectorStore._set_lexical_ready_safe = _set_lexical_ready_safe
