...
EXTRACTION_CACHE_VERSION = "pdf-extractor-v3"
...
class PDFExtractor:
    ...
    def _safe_image_stats(page: fitz.Page) -> tuple[int, float]:
        ...
        for img in images:
            try:
                for box in page.get_image_rects(img[0]):
    ...
    def _assess_page_ocr_need(cls, page: fitz.Page, text: str, *, min_char_count: int = 120, min_alpha_count: int = 40, min_word_count: int = 25, image_coverage_threshold: float = 0.55, image_heavy_word_limit: int = 30, min_char_density: float = 0.0008) -> dict[str, Any]:
        ...
        image_count, image_coverage_pixels = cls._safe_image_stats(page)
        ...
        if alpha_count < min_alpha_count and word_count < min_word_count:
            reasons.append(f"sparse_text (alpha={alpha_count}, words={word_count})")
        if coverage > image_coverage_threshold and word_count < image_heavy_word_limit:
            reasons.append(f"image_heavy (coverage={coverage:.2f})")
        if char_count >= min_char_count:
            printable = sum(1 for c in text if c.isprintable() or c in "\n\r\t") / max(char_count, 1)
            if printable < 0.85:
                reasons.append("garbled_text_layer")
        if density < min_char_density and word_count < min_word_count:
            reasons.append(f"low_char_density ({density:.5f})")
    ...
    @staticmethod
    def _extract_figure_captions_from_text(text: str) -> list[str]:
        captions: list[str] = []
        pattern = re.compile(r"^\s*(?:figure|fig\.?|illustration|image|photo|diagram|scheme|schéma)\b", re.I)
        for block in split_paragraphs(text or ""):
            for line in clean_text(block).splitlines():
                value = clean_text(line)
                if value and pattern.match(value) and value not in captions:
                    captions.append(value[:1000])
                    if len(captions) >= 20:
                        return captions
        return captions

    @staticmethod
    def _extract_figure_captions(page: fitz.Page) -> list[str]:
        captions: list[str] = []
        try:
        ...
        for block in blocks:
            ...
            for line in value.splitlines():
                ...
                if line and pattern.match(line) and line not in captions:
                    captions.append(line[:1000])
                    if len(captions) >= 20:
                        return captions
        return captions
    ...
    def extract_iter(self, pdf_path: str | Path, document_id: str | None = None) -> Iterator[PageExtraction]:
        ...
        try:
            try:
                ...
                raise RuntimeError(f"{pdf_file.name}: failed to open PDF") from exc
            ...
            for index in range(page_count):
                ...
                if (
                    ...
                    table_texts = self._extract_table_blocks_from_text(text)
                    figure_captions = self._extract_figure_captions_from_text(text)
                    meta.update({"physical_page": physical_page, "cached": True, "quality_score": quality.quality, "routing_decision": quality.route, "table_texts": table_texts, "figure_captions": figure_captions, **enrich_text(text)})
                    table_ids = list(meta.get("table_ids") or [f"{document_id}:p{physical_page}:table:{i + 1}" for i in range(len(table_texts))])
                    figure_count = int(meta.get("image_count") or 0)
                    figure_ids = list(meta.get("figure_ids") or [f"{document_id}:p{physical_page}:figure:{i + 1}" for i in range(max(figure_count, len(figure_captions)))])
                    if self.state_store:
                        self.state_store.update_document(document_id, current_stage="EXTRACTING", current_page=physical_page, total_pages=page_count)
                    yield PageExtraction(document_id, pdf_file.name, index, physical_page, text, cached.get("extraction_method") or "cached", ocr_status=cached.get("ocr_status", "not_required"), page_type=meta.get("page_type", "unknown"), image_count=figure_count, table_count=len(table_texts), has_images=bool(meta.get("has_images") or figure_count), blocks=[p.strip() for p in split_paragraphs(text) if p.strip()], metadata=meta, source_path=str(pdf_file), quality_score=quality.quality, routing_decision=quality.route, table_ids=table_ids, figure_ids=figure_ids, table_texts=table_texts, figure_captions=figure_captions, normalized_text=meta.get("normalized_text", ""), entities=meta.get("entities", {}), headings=meta.get("headings", []))
                    continue
                ...
                table_blocks = [x.strip() for x in tables_text.split("\n\n") if x.strip()] if tables_text else []
