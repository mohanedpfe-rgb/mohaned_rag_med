...
class PDFExtractor:
    """Universal PDF extractor: native text, tables, figures and adaptive OCR with quality telemetry."""
    ...
    def _looks_duplicate(cls, existing: str, candidate: str) -> bool:
        a = cls._normalize_for_dedupe(existing)
        b = cls._normalize_for_dedupe(candidate)
        if not a or not b:
            return False
    ...
    def _merge_native_and_ocr_text(cls, native_text: str, ocr_text: str) -> str:
        ...
        merged: list[str] = []
        for part in parts:
            if not any(cls._looks_duplicate(existing, part) for existing in merged):
                merged.append(part)
        return clean_text("\n\n".join(merged)) if merged else clean_text(native_text or ocr_text or "")

    @staticmethod
    def _assess_page_ocr_need(page: fitz.Page, text: str, *, min_char_count: int = 120, min_alpha_count: int = 40, min_word_count: int = 25, image_coverage_threshold: float = 0.55, image_heavy_word_limit: int = 30, min_char_density: float = 0.0008) -> dict[str, Any]:
        word_count = len((text or "").split())
        char_count = len(text or "")
        alpha_count = sum(1 for c in text if unicodedata.category(c).startswith(("L", "N")))
        image_count, image_coverage_pixels = self._safe_image_stats(page)
        area = page.rect.width * page.rect.height if page.rect.width and page.rect.height else 1.0
        coverage = image_coverage_pixels / area
        ...
        if image_count and word_count < 60: page_type = "image_heavy"
        elif ocr_required: page_type = "scanned_or_ocr_required"
        elif word_count < 100: page_type = "sparse_text"
        else: page_type = "text_based"
    ...
    def _safe_image_stats(page: fitz.Page) -> tuple[int, float]:
        ...
        except (ValueError, RuntimeError): return 0, 0.0
        coverage = 0.0
        for img in images:
            try:
    ...
    def extract_iter(self, pdf_path: str | Path, document_id: str | None = None) -> Iterator[PageExtraction]:
        ...
        try:
            ...
            except RuntimeError as exc: raise RuntimeError(f"{pdf_file.name}: failed to open PDF: {exc}") from exc
            ...
            for index in range(pdf.page_count):
                ...
                if cached and cached["extraction_status"] == "COMPLETED" and cached.get("text"):
                    ...
                    meta.update({"physical_page": physical_page, "cached": True, "quality_score": quality.quality, "routing_decision": quality.route, **enrich_text(text)})
                    ...
                    if self.state_store and (physical_page % 4 == 0 or physical_page == pdf.page_count) and physical_page != last_progress:
                ...
                try:
                    ...
                    page = self._load_page(pdf, index)
                    if not raw_text or not raw_text.strip(): raw_text = self._extract_page_text(page)
                except Exception:
                    page = self._load_page(pdf, index); raw_text = self._extract_page_text(page)
                text = clean_text(raw_text)
                assessment = self._assess_page_ocr_need(page, text, image_coverage_threshold=self.ocr_image_coverage_threshold, min_char_density=self.ocr_min_char_density)
                tables_text = self._extract_tables(page)
                if tables_text: text = clean_text(f"{text}\n\n[TABLE]\n{tables_text}")
                enrichment = enrich_text(text)
                quality = score_page_quality(text, page_number=physical_page, image_count=assessment["image_count"])
                ...
                table_blocks = [x for x in tables_text.split("\n\n") if x.strip()] if tables_text else []
                table_ids = [f"{document_id}:p{physical_page}:table:{i+1}" for i in range(len(table_blocks))]
                extraction = PageExtraction(
                    document_id=document_id, file_name=pdf_file.name, page_index=index, page_number=physical_page,
                    text=text, extraction_method="pdf_text", ocr_required=assessment["ocr_required"], ocr_status="not_required",
                    page_type=assessment["page_type"], image_count=assessment["image_count"], table_count=len(table_blocks),
                    has_images=assessment["has_images"], blocks=[p.strip() for p in split_paragraphs(text) if p.strip()],
                    metadata={
                        "word_count": assessment["word_count"], "char_count": assessment["char_count"], "alpha_count": assessment["alpha_count"],
                        "image_coverage": round(assessment["image_coverage"], 4), "char_density": round(assessment["char_density"], 6),
                        "physical_page": physical_page, "printed_page_number": printed_page_number, "ocr_reasons": assessment["reasons"],
                        "table_ids": table_ids, "figure_ids": figure_ids, "evidence_types": (["text"] if text else []) + (["table"] if table_blocks else []) + (["figure"] if figure_ids else []),
                        "quality_score": quality.quality, "routing_decision": quality.route, **enrichment,
                    }, source_path=str(pdf_file), quality_score=quality.quality, routing_decision=quality.route,
                    table_ids=table_ids, figure_ids=figure_ids, normalized_text=enrichment["normalized_text"], entities=enrichment["entities"], headings=enrichment["headings"],
                )
                    ...
                    if not self.ocr_enabled:
                        ...
                        try:
                            ...
                            if not ocr_text: extraction.ocr_status = "skipped_no_result"
                                ...
                                extraction.ocr_status = "skipped_low_confidence"; extraction.metadata["ocr_confidence"] = float(confidence)
                            else:
                                ocr_text = clean_text(ocr_text); native = extraction.text.strip(); final = self._merge_native_and_ocr_text(native, ocr_text)
                                if final != native: extraction.text = final; extraction.extraction_method = "ocr"; extraction.blocks = [p.strip() for p in split_paragraphs(final) if p.strip()]
                                extraction.ocr_status = "success"; extraction.ocr_confidence = confidence; extraction.ocr_required = True
                                extraction.metadata.update(enrich_text(extraction.text)); extraction.metadata["ocr_char_count"] = len(ocr_text)
                        except Exception as exc:
                            extraction.ocr_status = "failed"; extraction.metadata["ocr_error"] = str(exc); extraction.metadata["quality_warning"] = "Page requires OCR but OCR failed."
                if self.state_store and self.state_store.get_document(document_id):
                    ...
                    if (physical_page % 4 == 0 or physical_page == pdf.page_count) and physical_page != last_progress:
                        self.state_store.update_document(document_id, current_stage="EXTRACTING", current_page=physical_page); last_progress = physical_page
                yield extraction
        except Exception as exc:
            if pdf is None: raise RuntimeError(f"{pdf_file.name}: failed to open PDF: {exc}") from exc
            raise
        finally:
