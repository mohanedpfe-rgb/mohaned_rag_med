...
class PDFExtractor:
    ...
    def _assess_page_ocr_need(cls, page: fitz.Page, text: str, *, min_char_count: int = 120, min_alpha_count: int = 40, min_word_count: int = 25, image_coverage_threshold: float = 0.55, image_heavy_word_limit: int = 30, min_char_density: float = 0.0008) -> dict[str, Any]:
        ...
        if image_count and word_count < 60:
        ...
        try:
            ...
            for index in range(pdf.page_count):
                ...
                if cached and cached["extraction_status"] == "COMPLETED" and cached.get("text"):
                    ...
                    enriched_cached = enrich_text(text)
                    meta.update({"physical_page": physical_page, "cached": True, "quality_score": quality.quality, "routing_decision": quality.route, **enriched_cached})
                ...
                extraction = PageExtraction(
                    document_id=document_id, file_name=pdf_file.name, page_index=index, page_number=physical_page, text=text,
                    extraction_method="pdf_text", ocr_required=assessment["ocr_required"], ocr_status="not_required", page_type=assessment["page_type"],
                    image_count=assessment["image_count"], table_count=len(table_blocks), has_images=assessment["has_images"], blocks=[p.strip() for p in split_paragraphs(text) if p.strip()],
                    ...
                    source_path=str(pdf_file), quality_score=quality.quality, routing_decision=quality.route, table_ids=table_ids, figure_ids=figure_ids, normalized_text=enrichment["normalized_text"], entities=enrichment["entities"], headings=enrichment["headings"],
                )
