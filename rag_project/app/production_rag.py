...
def _extract_marker_lines(answer: str) -> list[tuple[str, int]]:
    ...
    for raw in str(answer or "").splitlines():
        line=raw.strip(); match=re.search(r"\[S(\d+)\]\s*$", line, re.I)
        if not match: continue
        text=re.sub(r"\s*\[S\d+\]\s*$", "", line, flags=re.I); text=re.sub(r"^[-*•\s]+", "", text).strip()
...
def _verify_extractive_provenance(answer: str, hits: list[Any]) -> dict[str, Any]:
    ...
    for text,source_no in rows:
        idx=source_no-1
        if idx<0 or idx>=len(hits): details.append({"source":source_no,"matched":False,"reason":"source_out_of_range"}); continue
        source_text=_norm_provenance_text(getattr(hits[idx],"text","")); needle=_norm_provenance_text(text); ok=bool(needle and len(needle)>=12 and needle in source_text); matched+=int(ok); details.append({"source":source_no,"matched":ok,"reason":"exact_source_substring" if ok else "source_mismatch"})
    ratio=matched/max(1,len(rows)); return {"allow":matched==len(rows) and matched>0,"supported_ratio":ratio,"method":"exact_extractive_provenance","matched":matched,"total":len(rows),"details":details}

def _recovery_trace(question: str, hits: list[Any], exc: Exception, grounding: dict[str, Any], method: str) -> dict[str, Any]: return {"mode":"grounded_recovery","question":str(question or "").strip(),"primary_pipeline_error":type(exc).__name__,"retrieval":{"status":"completed","final_hits":len(hits)},"generation":{"status":"extractive","method":method},"verification":{"status":"completed","method":method,"supported_ratio":float(grounding.get("supported_ratio",0.0) or 0.0)}}
...
def _record_answer_replay(system: Any, question: str, result: dict[str, Any]) -> None:
    try:
        ...
        if not root: return
        trace=result.get("query_trace") or {}; status=str(result.get("status") or "").upper(); replay={"success":status in {"SUCCESS","SUCCESS_WITH_WARNINGS"},"question":str(question or "")[:1200],"status":status,"failure_stage":"" if status in {"SUCCESS","SUCCESS_WITH_WARNINGS"} else (str((trace.get("verification") or {}).get("status") or "") or "runtime_or_grounding"),"routing":trace.get("routing") or {},"retrieval":trace.get("retrieval") or {},"generation":trace.get("generation") or {},"verification":trace.get("verification") or {},"latency_ms":trace.get("timings_ms",{}).get("total") if isinstance(trace.get("timings_ms"),dict) else None}
...
class ProductionRAGSystem(ResilientRAGSystem):
    ...
    def ingest_file(self,pdf_path):
        source=Path(pdf_path); result=versioned_ingestor.ingest_version_safely(self,source)
        if str((result or {}).get("status") or "").lower()=="skipped": result=self._archive_duplicate_upload(source,str((result or {}).get("document_id") or "unknown"),result)
        return result
    ...
    def _recovery_answer(self,question,metadata_filter,exc):
        try:
            from rag_project.retrieval.metadata_filter import MetadataFilter; where=MetadataFilter.build(metadata_filter)
        except Exception: where=None
        retriever=getattr(self,"retriever",None); retrieve=getattr(retriever,"retrieve",None)
        ...
        try: hits=[h for h in _safe_list(retrieve(str(question or "").strip(),top_k=max(1,min(int(getattr(self.settings,"top_k",8)),12)),where=where)) if h is not None]
        ...
        if provenance.get("allow"):
            citations=[{**citation,"valid":True} if isinstance(citation,dict) and "valid" not in citation else citation for citation in citations]; grounding={"allow":True,"supported_ratio":1.0,"method":"exact_extractive_provenance","verified_items":provenance.get("details",[])}
    ...
    def answer(self,question:str,metadata_filter:Dict[str,Any]|None=None)->dict[str,Any]:
        ...
        _record_answer_replay(self,original_question,result); return result
