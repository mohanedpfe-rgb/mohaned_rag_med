...
def _stable_embed_batch(self: Any, texts: list[str]) -> list[list[float]]:
    ...
    The previous implementation could fall back to SentenceTransformers, which may
    trigger a large model download and make ingestion appear frozen. The explicit
    fallback for a failed semantic backend is handled by the ingestion pipeline.
    """
    if not texts:
        return []
    if self.test_mode:
    ...
    if not self._check_ollama_available(force=True):
        raise RuntimeError(
            f"Ollama embedding backend is unavailable at {self.base_url!r}. "
...
def _stable_set_version_state(self: Any, document_id: str, version_id: str, state: str) -> None:
    ...
    with sqlite3.connect(self.lexical_database) as connection:
        connection.execute(
            ...
            SET index_state = ?, metadata = json_set(metadata, '$.index_state', ?)
            WHERE json_extract(metadata, '$.document_id') = ?
              AND json_extract(metadata, '$.version_id') = ?
            """,
            (normalized, normalized, str(document_id), str(version_id)),
        )
        connection.commit()
...
def _stable_validate_document_index(
    self: Any,
    document_id: str,
    ...
    *,
    ...
    records = self.collection.get(where={"document_id": document_id}, include=["metadatas"])
    ids = [str(item_id) for item_id in records.get("ids", []) or []]
    metadatas = [self._coerce_metadata(item) for item in records.get("metadatas", []) or []]
    ...
    if chosen_ids:
        try:
            ...
            embeddings = sampled.get("embeddings") or []
            ...
            issues.append(f"embedding validation sample failed: {exc}")
...
def _stable_ingest_file(self: Any, pdf_path: str | Path) -> dict[str, Any]:
    """Production ingestion state machine with bounded work and recovery at every phase."""
    ...
    heartbeat = threading.Thread(
        ...
        args=(self, document_id, worker_id, stop_heartbeat),
        name=f"ingest-heartbeat-{document_id[:8]}",
        daemon=True,
    )
    heartbeat.start()
    ...
    try:
        ...
        self.state_store.record_event(
            ...
            message=f"Validated {source.name}: {page_count} pages",
        ...
        self.state_store.record_event(
            ...
            status="RUNNING",
            event_type="extract",
            message=f"Extracting text page-by-page: 0/{page_count}",
            current_page=0,
            total_pages=page_count,
            file_name=source.name,
        )
        ...
        extractor = PDFExtractor(
            self.state_store,
            ocr_enabled=bool(getattr(self.settings, "ocr_enabled", True)),
            ocr_confidence_threshold=float(getattr(self.settings, "ocr_confidence_threshold", 0.55)),
            ocr_min_char_density=float(getattr(self.settings, "ocr_min_char_density", 0.001)),
            ocr_image_coverage_threshold=float(getattr(self.settings, "ocr_image_coverage_threshold", 0.55)),
        )
        ...
        for batch_number, batch in enumerate(batches, start=1):
            ...
            if first_batch:
                ...
                self.state_store.record_event(
                    ...
                    status="RUNNING",
                    event_type="chunk",
                    message="Splitting extracted pages into searchable sections",
                    details={"chunk_size": self.settings.chunk_size, "chunk_overlap": self.settings.chunk_overlap, "page_batch_size": self.settings.page_batch_size},
                    current_page=0,
                    total_pages=page_count,
                    file_name=source.name,
                )
            ...
            page_numbers = [int(page) for chunk in batch for page in (chunk.page_numbers or [])]
            ...
            if semantic_enabled:
                ...
                self.state_store.record_event(
                    ...
                    message=f"Embedding batch {batch_number}: {len(documents)} chunks through page {current_page}/{page_count}",
                    details={"batch": batch_number, "chunk_batch_size": len(documents)},
                ...
                try:
                    ...
                    self.state_store.record_event(
                        ...
                        message="Semantic embeddings unavailable; continuing with lexical indexing so ingestion can finish.",
            ...
            self.state_store.update_document(
                ...
                current_page=current_page,
                total_pages=page_count,
                embedding_dimension=self.embedding_service.dimension if semantic_enabled else None,
            ...
            self.state_store.record_event(
                ...
                message=f"Batch {batch_number} committed: {chunk_count} chunks through page {current_page}/{page_count}",
        ...
        if source.resolve() != target.resolve():
            if target.exists():
                archived = self.settings.archive_dir / source.name
                if archived.exists():
                    archived = self.settings.archive_dir / f"{source.stem}-{content_hash[:12]}{source.suffix}"
        ...
        if not semantic_enabled:
            warning = "Semantic embeddings were unavailable; lexical search is active for this document."
        self.state_store.record_event(
            document_id,
            stage="READY",
            status="READY",
            event_type="completion",
            message=f"Document ready: {page_count} pages, {chunk_count} chunks, {embedding_count} embeddings",
            details={"page_count": page_count, "chunk_count": chunk_count, "embedding_count": embedding_count, "retrieval_mode": "hybrid" if semantic_enabled else "lexical", "warning": warning},
        ...
        failed = self.settings.failed_dir / source.name
        try:
...
def install() -> None:
    ...
    # Restore the native page extractor after the expensive per-page subprocess patches.
    if hasattr(PDFExtractor, "_original_extract_iter"):
        PDFExtractor.extract_iter = _stable_extract_iter

    # Disable the hidden SentenceTransformers auto-download fallback during ingestion.
    ...
    # Local Chroma + SQLite writes are serialized intentionally.
    ...
    # Cap embedding waits on newly constructed services without changing the public API.
