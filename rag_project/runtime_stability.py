...
def _stable_ingest_file(self: Any, pdf_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        self.state_store.transition_document_state(document_id, "READY", current_page=page_count, total_pages=page_count, ingestion_completed_at=_now(), file_path=str(target.resolve()), embedding_dimension=self.embedding_service.dimension if semantic_enabled else None, version_id=version_id, index_state="READY", error=warning, chunk_count=chunk_count, embedding_count=embedding_count, ingestion_metrics=json.dumps({"page_count": page_count, "chunk_count": chunk_count, "embedding_count": embedding_count, "retrieval_mode": "hybrid" if semantic_enabled else "lexical", "total_ms": round((time.perf_counter() - started) * 1000, 3), "extraction": round(extraction_ms, 3)}, sort_keys=True))
