...
def robust_ingest_file(system: Any, pdf_path: str | Path) -> dict[str, Any]:
    ...
    # The atomic helper already owns the lease. Re-enter the normal claim wrapper
    # only to register the worker in runtime_stability_v4's in-process fence map.
    ...
    try:
        ...
        for batch_number, batch in enumerate(chunk_batches, start=1):
            ...
            for offset, chunk in enumerate(batch):
                global_index = chunk_count + offset; chunk.chunk_index = global_index
                chunk_id = f"{document_id}-{content_hash[:12]}-{global_index}"; ids.append(chunk_id)
