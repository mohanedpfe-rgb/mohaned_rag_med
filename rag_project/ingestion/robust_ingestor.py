...
def robust_ingest_file(system: Any, pdf_path: str | Path) -> dict[str, Any]:
    ...
    current_chunking_config = json.dumps(
        {"size": system.settings.chunk_size, "overlap": system.settings.chunk_overlap},
        sort_keys=True,
    )
    ...
    stat = file_path.stat()
    system.state_store.upsert_document(
        {
    ...
    if not system.state_store.claim_document(
        document_id,
        worker_id,
    ...
    try:
        ...
        system.state_store.record_event(
            ...
            message=f"Classified {file_path.name}: {classification.get('document_type', 'unknown')} ({total_pages} pages)",
        ...
        system.state_store.record_event(
            ...
            status="RUNNING",
            event_type="extract",
            message=f"Reading {total_pages} PDF pages one by one",
        ...
        system.state_store.record_event(
            ...
            message=f"Finalizing {chunk_count} indexed chunks",
        ...
        system.state_store.record_event(
            ...
            message="Index integrity passed; promoting this version to searchable state",
            details={"version_id": content_hash, "chunk_count": chunk_count, "embedding_count": embedding_count},
            current_page=total_pages,
            total_pages=total_pages,
            file_name=file_path.name,
        )
        ...
        if target.exists() and not same_target:
            ...
            if archived.exists():
                archived = system.settings.archive_dir / f"{file_path.stem}-{content_hash[:12]}{file_path.suffix}"
        ...
        system.state_store.transition_document_state(
            ...
            ingestion_metrics=json.dumps(metrics, sort_keys=True),
        )
