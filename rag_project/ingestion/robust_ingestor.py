...
def robust_ingest_file(system: Any, pdf_path: str | Path) -> dict[str, Any]:
    ...
    # Register the same lease in the runtime fence map. The atomic helper has
    # already acquired ownership, so this second UPDATE cannot displace another worker.
    ...
    try:
        # Retry cleanup is performed only after this worker owns the document.
        ...
        if previous and previous.get("content_hash") != content_hash:
            ...
            try:
                system.vector_store.delete_version(document_id, previous.get("content_hash"))
            except Exception:
                system.logger.exception("Failed to clean previous index version for %s", file_path.name)
        ...
        chunk_count = 0; embedding_count = 0; document_language: str | None = None
        embedding_ms = 0.0; indexing_ms = 0.0; current_page = 0; embedding_stage_started = False
        for batch_number, batch in enumerate(chunk_batches, start=1):
            ...
            if not embedding_stage_started:
                ...
                if system.embedding_startup_error is not None: raise RuntimeError(f"FAILED_EMBEDDING: {system.embedding_startup_error}")
                ...
                if compatibility.get("status") != "READY": raise RuntimeError(f"FAILED_EMBEDDING: {compatibility.get('message', 'embedding index incompatible')}")
                ...
                mark("chunking"); embedding_stage_started = True
            if document_language is None: document_language = detect_language(" ".join(chunk.text for chunk in batch))
            documents = [chunk.text for chunk in batch]; metadatas: list[dict[str, Any]] = []; ids: list[str] = []
            for offset, chunk in enumerate(batch):
                global_index = chunk_count + offset; chunk.chunk_index = global_index; chunk_id = f"{document_id}-{content_hash[:12]}-{global_index}"; ids.append(chunk_id)
            ...
            try:
                ...
                if len(vectors) != len(documents): raise RuntimeError(f"Embedding backend returned {len(vectors)} vectors for {len(documents)} chunks")
            ...
            embedding_ms += (time.perf_counter() - embedding_started) * 1000
            renew(force=True); check_cancel()
            indexing_started = time.perf_counter(); system.vector_store.add_documents(documents, metadatas, vectors, ids); indexing_ms += (time.perf_counter() - indexing_started) * 1000
            chunk_count += len(batch); embedding_count += len(vectors)
            ...
            renew()
        check_cancel(); renew(force=True)
        if chunk_count == 0 or embedding_count == 0: raise ValueError(f"No extractable searchable content was produced for {file_path.name}.")
        ...
        system.state_store.record_event(document_id, stage="INDEXING", status="RUNNING", event_type="index_write", message=f"Finalizing {chunk_count} indexed chunks", details={"chunk_count": chunk_count, "embedding_count": embedding_count}, current_page=total_pages, total_pages=total_pages, file_name=file_path.name); mark("embedding_and_incremental_index")
        validation = system.vector_store.validate_document_index(document_id, content_hash)
        if not validation.get("valid") or validation.get("count") != embedding_count: raise RuntimeError("FAILED_EMBEDDING: committed index validation failed: " + "; ".join(validation.get("issues", [])))
        system.vector_store.set_version_index_state(document_id, content_hash, "READY")
        if previous_version:
            system.vector_store.set_version_index_state(document_id, previous_version, "FAILED"); system.vector_store.delete_version(document_id, previous_version)
        ...
        system.state_store.record_event(document_id, stage="VALIDATING_INDEX", status="RUNNING", event_type="validation", message="Index integrity passed; promoting this version to searchable state", details={"version_id": content_hash, "chunk_count": chunk_count, "embedding_count": embedding_count}, current_page=total_pages, total_pages=total_pages, file_name=file_path.name); mark("validation")
        target = system.settings.processed_dir / file_path.name; same_target = file_path.resolve() == target.resolve()
        if target.exists() and not same_target:
            archived = system.settings.archive_dir / file_path.name
            if archived.exists(): archived = system.settings.archive_dir / f"{file_path.stem}-{content_hash[:12]}{file_path.suffix}"
            archived.parent.mkdir(parents=True, exist_ok=True); target.replace(archived)
        if not same_target: file_path.replace(target)
        system.state_store.record_event(document_id, stage="READY", status="READY", event_type="completion", message="Document ready for retrieval and grounded questions", details={"page_count": total_pages, "chunk_count": chunk_count, "embedding_count": embedding_count, "vector_store_count": system.vector_store.count()}, current_page=total_pages, total_pages=total_pages, file_name=file_path.name); mark("file_finalize")
        ...
        system.state_store.transition_document_state(document_id, "READY", ingestion_completed_at=utc_now(), current_page=total_pages, total_pages=total_pages, file_path=str(target.resolve()), ingestion_metrics=json.dumps(metrics, sort_keys=True))
        ...
        try:
            system.vector_store.delete_version(document_id, content_hash); system.vector_store.set_version_index_state(document_id, content_hash, "FAILED")
        except Exception: system.logger.exception("Failed to remove partial index for %s", file_path.name)
        ...
        except Exception: system.state_store.update_document(document_id, current_stage=failure_stage, status=failure_stage, error=str(exc))
        failed_path = system.settings.failed_dir / file_path.name
        if file_path.exists() and file_path.resolve() != failed_path.resolve():
            try:
                failed_path.parent.mkdir(parents=True, exist_ok=True); shutil.copy2(file_path, failed_path)
            except OSError: system.logger.exception("Failed to quarantine %s", file_path.name)
        return {"status": "failed", "file_name": file_path.name, "document_id": document_id, "error": str(exc)}
    finally:
        system.state_store.release_document(document_id, worker_id); system._remove_cancel_flag(document_id)
