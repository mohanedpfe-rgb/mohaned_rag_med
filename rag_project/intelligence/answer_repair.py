...
def repair_needed(checks:Sequence[ClaimCheck])->bool:
    """Trigger exactly one bounded repair for any claim that is not fully supported."""
...
def repair_and_verify(llm,question,draft,evidence_blocks,source_ids,checks):
    ...
    # Never accept a repair that introduces a blocked claim. The caller can then
    # fall back to the original or citation-firewall path.
    blocked=[c for c in fresh if c.status in {'UNSUPPORTED','NUMERIC_MISMATCH','CONTRADICTED'} or c.contradiction]
    if blocked:
