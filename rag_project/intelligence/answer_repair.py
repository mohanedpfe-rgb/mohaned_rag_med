...
def repair_needed(checks:Sequence[ClaimCheck])->bool: return any(c.status!='SUPPORTED' or c.contradiction for c in checks)
...
def repair_and_verify(llm,question,draft,evidence_blocks,source_ids,checks):
    ...
    return repaired,verify_claims(repaired,evidence_blocks,source_ids),True
