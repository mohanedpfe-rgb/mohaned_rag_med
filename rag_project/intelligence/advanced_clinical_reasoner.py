...
def extract_clinical_facts(text:str,*,node_id:str='',document_id:str='')->tuple[ClinicalFact,...]:
    ...
    for sentence in re.split(r'(?<=[.!?。！？])\s+|\n+',str(text or '').strip()):
        ...
        ents=list(extract_clinical_entities(sentence)); rels=_relation_hits(sentence)
    ...
    for start,target in zip(q,q[1:]):
        ...
        while stack:
            ...
            for f in adj.get(node,()):
                ...
                nd=set(docs); nd.add(f.document_id) if f.document_id else None
...
def _safety_conflict(u:QueryUnderstanding,facts:Sequence[ClinicalFact],texts:Sequence[str]=())->float:
    ...
    contra=max(contra,1 if re.search(r'\b(contraindicated|contre-indiqué|ممنوع|تجنب)\b',combined,re.I|re.UNICODE) else 0)
...
def assess_clinical_reasoning(understanding:QueryUnderstanding,hits:Sequence[Any],*,max_depth:int=3)->ClinicalReasoningAssessment:
    ...
    for i,h in enumerate(hits):
        meta=getattr(h,'metadata',{}) or {}; text=str(getattr(h,'text','') or ''); texts.append(text); facts.extend(extract_clinical_facts(text,node_id=str(meta.get('chunk_id') or f'N{i+1}'),document_id=str(meta.get('document_id') or getattr(h,'doc_id','')))); qe={e.normalized for e in understanding.entities}; ee={e.normalized for e in extract_clinical_entities(text)}; coverage.append(len(qe&ee)/max(1,len(qe)))
...
def build_reasoning_instruction(understanding:QueryUnderstanding,assessment:ClinicalReasoningAssessment)->str:
    if assessment.mode=='MULTI_HOP': return 'Use only the explicit retrieved evidence path. Preserve uncertainty, population, timing, safety qualifiers, numbers, and units. Do not invent a missing link.'
    if assessment.mode=='ONE_HOP': return 'Use only the explicit relationship supported by retrieved evidence.'
