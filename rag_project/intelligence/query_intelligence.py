...
def _contains_term(text: str, term: str) -> bool:
    ...
    if re.match(r"[\u0600-\u06ff]",needle) and not needle.startswith("ال"):return bool(re.search(rf"(?<!\w)ال{re.escape(needle)}(?!\w)",haystack,flags=re.UNICODE))
    return False

def _contains_any(text:str,terms:tuple[str,...])->bool:return any(_contains_term(text,term) for term in terms)

@dataclass(frozen=True)
class QueryPlan:
    original:str; normalized:str; intent:str; subqueries:tuple[str,...]; variants:tuple[str,...]; entities:tuple[str,...]; needs_numeric:bool; needs_table:bool; needs_figure:bool; needs_multi_hop:bool; needs_neighborhood:bool; score_boosts:dict[str,float]
    def to_dict(self)->dict[str,Any]:return asdict(self)

def normalize_query(query:str)->str:
    raw=str(query or "").strip();text=re.sub(r"\s+"," ",raw).replace("–","-").replace("—","-")
    text=re.sub(r"^\s*follow[- ]up\s*:\s*","",text,flags=re.I)
    text=re.sub(r"\bwhat's\b","what is",text,flags=re.I);text=re.sub(r"\bvs\.?\b","versus",text,flags=re.I)
    if raw and raw[0].isupper() and text:text=text[0].upper()+text[1:]
    return text[:3000]

def extract_query_entities(query:str)->tuple[str,...]:
    understanding=understand_query(normalize_query(query));out=[]
...
def decompose_query(query:str)->tuple[str,...]:
    ...
    if len(pieces)==1 and re.search(r"\bversus\b|\bbetween\b",q,re.I):
        pieces=re.split(r"\bversus\b|\bbetween\b|\band\b",q,flags=re.I);pieces=[p.strip(" ?!;,." ) for p in pieces if p.strip()]
    return tuple(dict.fromkeys(pieces))[:8]

def classify_intent(normalized:str,subqueries:tuple[str,...])->str:
    semantic=understand_query(normalized);primary=semantic.primary_intent
    ...
    numeric_requested=_contains_any(normalized,_NUMERIC) or "numeric" in semantic.intents;management_requested=_contains_any(normalized,_MANAGEMENT)
    if len(subqueries)>1 and numeric_requested and management_requested:return "multi_part"
...
def _make_variants(normalized:str,intent:str,entities:tuple[str,...],*,numeric:bool,table:bool,figure:bool)->tuple[str,...]:
    ...
    if intent in {"numeric","diagnosis","management","multi_part"} or numeric:variants.extend([normalized+" exact numeric value units range",normalized+" dosage measurement quantity threshold"])
...
def plan_query(query:str,conversation_context:str="")->QueryPlan:
    original=query or "";normalized=normalize_query(original);semantic=understand_query(normalized,conversation_context=conversation_context);subqueries=decompose_query(normalized);entities=extract_query_entities(normalized)
    numeric="numeric" in semantic.intents or _contains_any(normalized,_NUMERIC);table="table_lookup" in semantic.intents or _contains_any(normalized,_TABLE) or (numeric and len(subqueries)>1);figure="figure_lookup" in semantic.intents or _contains_any(normalized,_FIGURE);relation="relationship" in semantic.intents or "association" in semantic.intents or _contains_any(normalized,_RELATION);navigation="navigation" in semantic.intents or _contains_any(normalized,_NAV);intent=classify_intent(normalized,subqueries)
