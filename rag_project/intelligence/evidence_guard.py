...
def detect_contradiction(claim,evidence_blocks:Sequence[str])->bool:
    cl=_score_text(claim).casefold();cl_tokens=set(meaningful_tokens(cl));blocks=[evidence_blocks] if isinstance(evidence_blocks,str) else list(evidence_blocks or ());generic={'patient','the','is','has','with','present','presence','absent','absence','not','no'}
    for ev in blocks:
        el=str(ev or '').casefold();ev_tokens=set(meaningful_tokens(el));shared=(cl_tokens&ev_tokens)-generic;explicit=any(re.search(a,cl,re.I) and re.search(b,el,re.I) for a,b in OPPOSITES);polarity=bool(NEG.search(cl))!=bool(NEG.search(el)) and bool(shared)
        if (explicit or polarity) and (semantic_support(cl,el)>=.08 or len(shared)>=1):return True
    return False

def _best_support(claim,blocks,ids):
    rows=sorted(((semantic_support(claim,b),ids[i] if i<len(ids) else f'S{i+1}') for i,b in enumerate(blocks)),reverse=True);rows=[r for r in rows if r[0]>0]
