...
SCALE = {
    "ug": ("mass", 1e-6), "mcg": ("mass", 1e-6), "mg": ("mass", 1e-3), "g": ("mass", 1), "kg": ("mass", 1000),
    "ml": ("volume", 1), "l": ("volume", 1000), "mmhg": ("pressure", 1), "cmh2o": ("pressure", 0.735559),
    "mmol/l": ("amount_concentration", 1), "mol/l": ("amount_concentration", 1000), "iu": ("activity", 1), "%": ("percent", 1),
    "bpm": ("rate", 1), "c": ("temperature", 1), "°c": ("temperature", 1), "mm": ("length", 1), "cm": ("length", 10), "m": ("length", 1000),
    "hz": ("frequency", 1), "khz": ("frequency", 1000), "m/s": ("velocity", 1), "s": ("time", 1), "min": ("time", 60), "h": ("time", 3600),
    "day": ("time", 86400), "days": ("time", 86400), "week": ("time", 604800), "weeks": ("time", 604800), "month": ("time", 2592000),
    "months": ("time", 2592000), "year": ("time", 31536000), "years": ("time", 31536000),
}
...
_CONCEPT_SYNONYMS = (
    (r"\bhyperglyc(?:emia|émie)\b|\bhyperglycemia\b", "hyperglycemia"), (r"\bcétose\b|\bketosis\b", "ketosis"),
    (r"\bacidose métabolique\b|\bmetabolic acidosis\b", "metabolic acidosis"), (r"\bhypokali(?:emia|émie)\b|\bhypokalemia\b", "hypokalemia"),
    (r"\bacidocétose diabétique\b|\bdiabetic ketoacidosis\b", "diabetic ketoacidosis"), (r"\bnéphropathie diabétique\b|\bnephropathie diabetique\b|\bdiabetic nephropathy\b", "diabetic nephropathy"),
    (r"\bdiabète\b|\bdiabete\b|\bdiabetes mellitus\b", "diabetes"), (r"\bcomplication microvasculaire\b|\bmicrovascular complication\b", "microvascular complication"),
    (r"\bchronique\b|\bchronic\b", "chronic"), (r"\bdu diabète\b|\bof diabetes\b", "of diabetes"),
)
...
def numeric_consistency(claim, evidence): return NumericConsistencyResult(numeric_consistency_details(claim, evidence))
...
def semantic_support(claim, evidence):
    ...
    if not claim or not evidence: return 0.0
...
def detect_contradiction(claim, evidence_blocks):
    cl = _score_text(claim).casefold(); blocks = [evidence_blocks] if isinstance(evidence_blocks, str) else list(evidence_blocks or ())
    cl_tokens = set(meaningful_tokens(cl)); generic = {"patient", "the", "is", "has", "with", "present", "presence", "absent", "absence", "not", "no"}
    for ev in blocks:
        el = str(ev or "").casefold(); shared = (cl_tokens & set(meaningful_tokens(el))) - generic
        ...
        if (explicit or polarity) and (len(shared) >= 1 or semantic_support(cl, el) >= 0.08): return True
    return False
...
def _cited_evidence(claim: str, evidence_blocks: Sequence[str], source_ids: Sequence[str]) -> tuple[list[str], list[str]]:
    ...
    if not markers: return list(evidence_blocks), list(source_ids)
    index_by_id = {str(source_id).casefold(): index for index, source_id in enumerate(source_ids)}; marker_keys = [marker.casefold() for marker in markers]
    if any(marker not in index_by_id for marker in marker_keys): return [], []
    selected_indices = [index_by_id[marker] for marker in marker_keys]; unique_indices = list(dict.fromkeys(selected_indices))
...
def verify_claims(answer, evidence_blocks: Sequence[str], source_ids: Sequence[str]) -> list[ClaimCheck]:
    ...
    for claim in split_claims(answer):
        cited_blocks, cited_ids = _cited_evidence(claim, evidence_blocks, source_ids); support_blocks = cited_blocks if cited_blocks else []; support_ids = cited_ids if cited_ids else []
...
def grounding_decision(claims: Sequence[ClaimCheck], min_supported_ratio: float = 0.70) -> dict[str, Any]:
    ...
    support_statuses = {"SUPPORTED", "PARTIAL", "ENTAILED"}
...
def contradiction_report(claims: Sequence[ClaimCheck]) -> dict[str, Any]:
    ...
    agreement = sum(1 for claim in rows if not claim.contradiction and claim.status in {"SUPPORTED", "PARTIAL", "ENTAILED"}) / max(1, len(rows))
    return {"has_contradiction": bool(conflicts), "count": len(conflicts), "conflict_count": len(conflicts), "conflicts": conflicts, "agreement": round(agreement, 4)}
...
def citation_firewall(answer: str, claims: Sequence[ClaimCheck]) -> tuple[str, bool]:
    rows = list(claims or ())
    safe_statuses = {"SUPPORTED", "PARTIAL", "ENTAILED"}
    unsafe = [claim for claim in rows if claim.status not in safe_statuses or claim.numeric_mismatch or claim.contradiction]
    if not unsafe: return str(answer or ""), False
    safe_claims = [claim.claim.strip() for claim in rows if claim not in unsafe and claim.claim.strip()]
    if safe_claims: return "\n".join(f"- {claim}" for claim in safe_claims) + "\n\n[Some claims were withheld because they could not be verified against the evidence.]", True
