...
def detect_contradiction(claim,evidence_blocks:Sequence[str])->bool:
    ...
    for ev in evidence_blocks:
        el=ev.casefold();opposite=any(re.search(a,cl,re.I) and re.search(b,el,re.I) for a,b in OPPOSITES) or (_polarity(cl)!=_polarity(el) and set(meaningful_tokens(cl))&set(meaningful_tokens(el)))
