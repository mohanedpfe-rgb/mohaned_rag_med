...
def _patch_structure_parser() -> None:
    ...
    def enrich(metadata: dict[str, Any], document: str, item_id: str = "") -> dict[str, Any]:
        ...
        for field in ("chapter_id", "chapter", "section_id", "section", "parent_id", "page_type", "ocr_status", "quality_score", "structure_version"):
        ...
        try:
            for page in original(self, pdf_path, document_id):
                ...
                try:
                    ...
                    quality = score_page_quality(page.text, page_number=physical, image_count=int(page.image_count or 0), page_area=page_width * page_height)
                    ...
                    if self.state_store is not None:
                        ...
                        payload.update({"quality_score": page.quality_score, "routing_decision": page.routing_decision, "quality_route": quality.route, "quality_warnings": list(quality.warnings), "figure_regions": figure_regions, "semantic_figure_count": len(page.figure_ids)})
...
def _patch_final_chunk_hierarchy() -> None:
    """Make the last-installed PDF layer reassert one page hierarchy anchor."""
    if getattr(SemanticChunker, "_final_pdf_v4_chunker_patched", False):
        return
    original = SemanticChunker.chunk_pages

    def chunk_pages(self: SemanticChunker, pages):
        chunks = list(original(self, pages) or [])
        page_groups: dict[tuple[str, int], list[Any]] = {}
        for chunk in chunks:
            page_no = int((chunk.page_numbers or [1])[0])
            page_groups.setdefault((str(chunk.doc_id), page_no), []).append(chunk)
        for group in page_groups.values():
            eligible = [chunk for chunk in group if chunk.representation_type in {"canonical", "table", "figure_caption"}]
            if not eligible:
                continue
            anchor = eligible[0]
            anchor_parent = str((anchor.metadata or {}).get("parent_id") or anchor.parent_id or "")
            anchor_section = str((anchor.metadata or {}).get("section_id") or anchor.section_id or "")
            anchor_chapter = str((anchor.metadata or {}).get("chapter_id") or "")
            anchor_path = [anchor_chapter, anchor_section, anchor_parent]
            for chunk in eligible:
                ...
                if any(anchor_path):
                    meta["hierarchy_path"] = anchor_path
                chunk.metadata = meta
    ...
    SemanticChunker._final_pdf_v4_chunker_patched = True
