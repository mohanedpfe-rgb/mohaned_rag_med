...
def _claim_texts(result:dict[str,Any])->list[str]: return [str(item.get("claim","")) for item in result.get("claims",[]) if isinstance(item,dict) and str(item.get("claim","")).strip()]
def _validated_model_entities(result:dict[str,Any])->list[str]:
    assist=_safe_dict(result.get("small_model_assist")); raw=[str(x).strip() for x in assist.get("entities",[]) if str(x).strip()]; evidence=" ".join(str(getattr(h,"text","") or "") for h in result.get("hits",[])).casefold(); return [x for x in raw if meaningful_tokens(x) and x.casefold() in evidence][:12]
def _sentence_units(answer:str)->list[str]:
    raw=str(answer or "").strip()
    if not raw:return []
    raw=re.sub(r"(?<=[.!?؟])\s+(?=\[S\d+\]\s*$)","",raw,flags=re.I|re.MULTILINE); units=[]
...
def _exact_provenance(answer:str,hits:list[Any])->dict[str,Any]:
    ...
    for raw in str(answer or "").splitlines():
        ...
        text=re.sub(r"\s*\[S\d+\]\s*$","",raw,flags=re.I); text=re.sub(r"^[-*•\s]+","",text).strip()
        if text:rows.append((text,int(match.group(1))))
    details=[]; matched=0
    for text,source_no in rows:
        idx=source_no-1
        if idx<0 or idx>=len(hits):details.append({"claim":text,"source":f"S{source_no}","matched":False,"reason":"source_out_of_range"}); continue
        source=re.sub(r"\s+"," ",str(getattr(hits[idx],"text","") or "")).strip().casefold(); needle=re.sub(r"\s+"," ",text).strip().casefold(); ok=len(needle)>=12 and needle in source; matched+=int(ok); details.append({"claim":text,"source":f"S{source_no}","matched":ok,"reason":"exact_source_substring" if ok else "source_mismatch"})
    return {"allow":bool(rows) and matched==len(rows),"supported_ratio":matched/max(1,len(rows)),"items":details}
def _extractive_answer(question:str,hits:list[Any],route:dict[str,Any],max_sentences:int)->tuple[str,list[dict[str,Any]]]:
    q_tokens=set(meaningful_tokens(question)); q_tokens-={"what","are","the","main","findings","is","this","that","does","document","report","explain","define","about","please","give","show","list","dans","les","des","une","un","est","que","quels","quelles","principales","résultats","ma","هو","هي","عن","هذه","هذا","ما","do"}; ranked=[]; summary=bool(route.get("summary"))
    for source_index,hit in enumerate(hits,1):
        text=str(getattr(hit,"text","") or ""); base=max(0.,min(1.,float(getattr(hit,"score",0.) or 0.)))
        for sentence in re.split(r"(?<=[.!?؟])\s+|\n+",text):
            sentence=re.sub(r"\s+"," ",sentence).strip(); sentence=re.sub(r"^\[(?:Chapter|Section):[^\]]*\]\s*","",sentence,flags=re.I).strip()
            if len(sentence)<20:continue
            overlap=len(set(meaningful_tokens(sentence))&q_tokens)/max(1,len(q_tokens)) if q_tokens else 0.; structural=.10 if summary and re.search(r"\b(introduction|overview|conclusion|diagnosis|etiology|clinical|treatment|chapter|section|rappel|exploration|diagnostic|traitement|étiologie|findings)\b",sentence,re.I) else 0.; ranked.append((.64*base+.30*overlap+structural,sentence,source_index))
    ranked.sort(key=lambda x:x[0],reverse=True); chosen=[]; seen_text=set(); seen_sources=set()
    for score,sentence,source_no in ranked:
        key=sentence.casefold()
        if key in seen_text or score<.10 or (summary and source_no in seen_sources):continue
        chosen.append((sentence,source_no)); seen_text.add(key); seen_sources.add(source_no)
        if len(chosen)>=max_sentences:break
    answer="\n".join(f"- {sentence} [S{source_no}]" for sentence,source_no in chosen); claims=[{"claim":sentence,"source":f"S{source_no}","provenance":"exact_sentence_from_retrieved_chunk"} for sentence,source_no in chosen]; return answer,claims
...
def _diagnostic_enhance(system:Any,question:str,result:dict[str,Any],metadata_filter:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    try:
        hits=list(result.get("hits") or []); plan=_safe_dict(result.get("query_analysis")); understanding=_safe_dict(result.get("semantic_understanding")); budget=choose_retrieval_budget(query_tokens=len(meaningful_tokens(question)),entity_count=len(plan.get("entities") or ()),intent=str(plan.get("intent","")),confidence=float(understanding.get("confidence",0.) or 0.),initial_score=float(_safe_dict(result.get("semantic_alignment")).get("score",0.) or 0.)); claims=_claim_texts(result); matrix=build_claim_evidence_matrix(claims,hits,[f"S{i+1}" for i in range(len(hits))]) if claims and hits else (); hierarchy=build_evidence_hierarchy(hits); grounding=_safe_dict(result.get("grounding")); contradiction=_safe_dict(result.get("contradiction_report")); retrieval_meta=_safe_dict(result.get("retrieval_quality")); calibration=calibrate_confidence(retrieval=max((float(getattr(h,"score",0.) or 0.) for h in hits),default=0.),rerank=max((float(getattr(h,"score",0.) or 0.) for h in hits),default=0.),entailment=float(grounding.get("supported_ratio",0.) or 0.),entity_coverage=float(_safe_dict(result.get("entity_coverage")).get("coverage",1.) or 1.),source_agreement=float(contradiction.get("agreement",1.) or 1.),contradiction=1. if contradiction.get("has_contradiction") else 0.,safety_conflict=float(_safe_dict(result.get("advanced_reasoning")).get("safety_conflict",0.) or 0.)).to_dict(); calibration["coverage"]=float(retrieval_meta.get("evidence_coverage",0.) or 0.); answer=str(result.get("answer") or ""); final_verification=verify_final_answer(answer,hits) if answer and hits else {"checked":True,"allow":False,"claim_count":0,"blocked_claims":0,"supported_ratio":0.,"evidence_claim_matrix":[]}; final_verification=dict(final_verification); final_verification.setdefault("claim_checks",claims); enhanced["final_verification"]=final_verification; enhanced.update({"evidence_claim_matrix":[r.to_dict() for r in matrix],"evidence_hierarchy":[r.to_dict() for r in hierarchy[:100]],"evidence_context_levels":select_context_levels(hierarchy),"adaptive_retrieval_budget":budget.to_dict(),"validated_small_model_entities":_validated_model_entities(result),"confidence_calibration":calibration,"confidence":{"level":calibration.get("level","low"),"evidence_confidence":calibration.get("calibrated",0.)},"god_mode_100":True,"evidence_first":bool(result.get("evidence_first",True)),"document_aware":bool(result.get("document_aware",True)),"pipeline_authority":PIPELINE_AUTHORITY})
        if not final_verification.get("allow",False):enhanced.update({"abstained":True,"abstention_reasons":["final_answer_verification_failed"],"status":"REASONING_ABSTAIN","citations":[],"answer":"I could not safely certify the requested answer against the indexed evidence."})
        else:enhanced["abstained"]=False
    except Exception:enhanced.setdefault("god_mode_100",True); enhanced.setdefault("evidence_first",True); enhanced.setdefault("pipeline_authority",PIPELINE_AUTHORITY)
    return enhanced
def _runtime_phase_implementation(result:dict[str,Any],hits:list[Any],final_matrix_or_checks:Any)->dict[str,Any]:
    raw=result if isinstance(result,dict) else {}; phases=raw.get("phases") if isinstance(raw.get("phases"),dict) else {}; verification=final_matrix_or_checks if isinstance(final_matrix_or_checks,dict) else {}; matrix=final_matrix_or_checks if isinstance(final_matrix_or_checks,list) else verification.get("evidence_claim_matrix",[]) or verification.get("claim_checks",[]) or []; claim_count=len(matrix) if isinstance(final_matrix_or_checks,list) else int(verification.get("claim_count",0) or len(matrix) or (len(hits) if verification else 0)); blocked=int(verification.get("blocked_claims",0) or 0) if isinstance(verification,dict) else 0; checked=bool(verification.get("checked",True)) if isinstance(verification,dict) else True
...
def _normalize_grounding_text(value:str)->str:
    value=re.sub(r"\[S\d+\]"," ",str(value or ""),flags=re.I); value=value.casefold().replace("’","'"); value=re.sub(r"[^\w\s]"," ",value,flags=re.UNICODE); return re.sub(r"\s+"," ",value).strip()
def _exact_llm_citation_grounding(answer:str,hits:list[Any])->bool:
    rows=[]
    for raw in re.split(r"\n+",str(answer or "")):
        for match in re.finditer(r"\[S(\d+)\]",raw,re.I):rows.append((raw,int(match.group(1))))
    if not rows:return False
    for raw,source_no in rows:
        idx=source_no-1
        if idx<0 or idx>=len(hits):return False
        claim=re.sub(r"\s*\[S\d+\]","",raw,flags=re.I).strip(); claim=re.sub(r"^[-*•\s]+","",claim); source=str(getattr(hits[idx],"text","") or ""); normalized_claim=_normalize_grounding_text(claim); normalized_source=_normalize_grounding_text(source)
    ...
    return True
def _enhanced_god_answer_impl(self:Any,question:str,metadata_filter:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    settings=getattr(self,"settings",None); top_k=max(4,min(12,int(getattr(settings,"top_k",8)))) if settings is not None else 8; hits,retrieval_state=retrieve_document_aware(self,clean_question,metadata_filter,top_k=top_k); route=_safe_dict(retrieval_state.get("route")); coverage=_safe_dict(retrieval_state.get("coverage")); answer_plan=build_answer_plan(clean_question,route_query_from_state(route),coverage)
    ...
    if generation_path=="deterministic_extractive":verified=_exact_provenance(answer,selected); grounding={"allow":bool(verified.get("allow")),"supported_ratio":float(verified.get("supported_ratio",0.) or 0.),"method":"exact_retrieved_sentence_provenance","verified_items":verified.get("items",[])}
    else:
        try:
            from rag_project.intelligence.evidence_guard import verify_claims,grounding_decision
