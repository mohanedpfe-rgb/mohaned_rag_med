...
def _validated_model_entities(result: dict[str, Any]) -> list[str]:
    assist = _safe_dict(result.get("small_model_assist")); raw = [str(x).strip() for x in assist.get("entities", []) if str(x).strip()]
...
def _exact_provenance(answer: str, hits: list[Any]) -> dict[str, Any]:
    ...
    for text,source_no in rows:
        ...
        source=re.sub(r"\s+"," ",str(getattr(hits[idx],"text","") or "")).strip().casefold(); needle=re.sub(r"\s+"," ",text).strip().casefold(); ok=len(needle)>=12 and needle in source
        matched += int(ok); details.append({"claim":text,"source":f"S{source_no}","matched":ok,"reason":"exact_source_substring" if ok else "source_mismatch"})
...
def _extractive_answer(question: str, hits: list[Any], route: dict[str, Any], max_sentences: int) -> tuple[str,list[dict[str,Any]]]:
    q_tokens=set(meaningful_tokens(question)); q_tokens-={"what","are","the","main","findings","is","this","that","does","document","report","explain","define","about","please","give","show","list","dans","les","des","une","un","est","que","quels","quelles","principales","résultats","ma","هو","هي","عن","هذه","هذا","ما","do"}
    ...
    for source_index,hit in enumerate(hits,1):
        ...
        for sentence in re.split(r"(?<=[.!?؟])\s+|\n+",text):
            ...
            ranked.append((.64*base+.30*overlap+structural,sentence,source_index))
...
def _diagnostic_enhance(system: Any, question: str, result: dict[str,Any], metadata_filter: dict[str,Any]|None=None) -> dict[str,Any]:
    ...
    try:
        hits=list(result.get("hits") or []); plan=_safe_dict(result.get("query_analysis")); understanding=_safe_dict(result.get("semantic_understanding"))
        budget=choose_retrieval_budget(query_tokens=len(meaningful_tokens(question)),entity_count=len(plan.get("entities") or ()),intent=str(plan.get("intent","")),confidence=float(understanding.get("confidence",0.) or 0.),initial_score=float(_safe_dict(result.get("semantic_alignment")).get("score",0.) or 0.))
        claims=_claim_texts(result); matrix=build_claim_evidence_matrix(claims,hits,[f"S{i+1}" for i in range(len(hits))]) if claims and hits else ()
        hierarchy=build_evidence_hierarchy(hits); grounding=_safe_dict(result.get("grounding")); contradiction=_safe_dict(result.get("contradiction_report")); retrieval_meta=_safe_dict(result.get("retrieval_quality"))
        calibration=calibrate_confidence(retrieval=max((float(getattr(h,"score",0.) or 0.) for h in hits),default=0.),rerank=max((float(getattr(h,"score",0.) or 0.) for h in hits),default=0.),entailment=float(grounding.get("supported_ratio",0.) or 0.),entity_coverage=float(_safe_dict(result.get("entity_coverage")).get("coverage",1.) or 1.),source_agreement=float(contradiction.get("agreement",1.) or 1.),contradiction=1. if contradiction.get("has_contradiction") else 0.,safety_conflict=float(_safe_dict(result.get("advanced_reasoning")).get("safety_conflict",0.) or 0.)).to_dict(); calibration["coverage"]=float(retrieval_meta.get("evidence_coverage",0.) or 0.)
        answer=str(result.get("answer") or ""); final_verification=verify_final_answer(answer,hits) if answer and hits else {"checked":True,"allow":False,"claim_count":0,"blocked_claims":0,"supported_ratio":0.,"evidence_claim_matrix":[]}; final_verification=dict(final_verification); final_verification.setdefault("claim_checks",claims)
...
def _runtime_phase_implementation(result: dict[str,Any], hits:list[Any], final_matrix_or_checks:Any)->dict[str,Any]:
    raw=result if isinstance(result,dict) else {}; phases=raw.get("phases") if isinstance(raw.get("phases"),dict) else {}; verification=final_matrix_or_checks if isinstance(final_matrix_or_checks,dict) else {}
    ...
    claim_count=len(matrix) if isinstance(final_matrix_or_checks,list) else int(verification.get("claim_count",0) or len(matrix) or (len(hits) if verification else 0)); blocked=int(verification.get("blocked_claims",0) or 0) if isinstance(verification,dict) else 0; checked=bool(verification.get("checked",True)) if isinstance(verification,dict) else True
...
def enhanced_god_answer(self:Any, question:str, metadata_filter:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    if synthesized:
        try:
            ...
            candidate_hits=hits[:max(top_k*3,12)]; blocks=[str(getattr(h,"text","") or "") for h in candidate_hits]; checks=list(verify_claims(synthesized,blocks,[f"S{i+1}" for i in range(len(candidate_hits))])); ground=grounding_decision(checks,min_supported_ratio=.70) if checks else {"allow":False,"supported_ratio":0.0}
            markers=re.findall(r"\[S(\d+)\]",synthesized); cited_sources={int(x) for x in markers}; sentences=_sentence_units(synthesized); all_cited=bool(markers) and bool(sentences) and all(re.search(r"\[S\d+\]",sentence,re.I) for sentence in sentences); source_numbers_valid=all(1<=n<=len(candidate_hits) for n in cited_sources)
            exact_citation_support=_exact_llm_citation_grounding(synthesized,candidate_hits)
            if checks and ground.get("allow") and all_cited and source_numbers_valid and exact_citation_support:
                answer=synthesized; provenance_claims=[c.to_dict() for c in checks]; generation_path="local_llm_grounded"
        except Exception:
            pass
    ...
    if generation_path=="deterministic_extractive":
        verified=_exact_provenance(answer,selected); grounding={"allow":bool(verified.get("allow")),"supported_ratio":float(verified.get("supported_ratio",0.) or 0.),"method":"exact_retrieved_sentence_provenance","verified_items":verified.get("items",[])}
    else:
        try:
            from rag_project.intelligence.evidence_guard import verify_claims, grounding_decision
