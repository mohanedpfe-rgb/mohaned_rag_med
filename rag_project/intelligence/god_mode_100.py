...
def _synthesize_from_evidence(system: Any, question: str, evidence_bundle: str) -> str | None:
    ...
    try:
        value=llm.generate(prompt=prompt,system_prompt="Use only supplied book evidence.",temperature=0.0); return str(value or "").strip()[:10000] or None
...
def _enhanced_god_answer_impl(self:Any, question:str, metadata_filter:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    hits,retrieval_state=retrieve_document_aware(self,clean_question,metadata_filter,top_k=top_k); route=_safe_dict(retrieval_state.get("route")); coverage=_safe_dict(retrieval_state.get("coverage")); answer_plan=build_answer_plan(clean_question,route_query_from_state(route),coverage)
...
def enhance_result(system:Any,question:str,result:dict[str,Any]|None=None,metadata_filter:dict[str,Any]|None=None)->dict[str,Any]:
    if isinstance(result,dict) and any(k in result for k in ("status","answer","hits","citations")): return _diagnostic_enhance(system,question,result,metadata_filter)
    effective_filter=result if isinstance(result,dict) else metadata_filter; return enhanced_god_answer(system,question,effective_filter)
