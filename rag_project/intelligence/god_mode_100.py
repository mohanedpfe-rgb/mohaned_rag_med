...
def _validated_model_entities(result: dict[str, Any]) -> list[str]:
    assist = result.get("small_model_assist") or {};raw = [str(x).strip() for x in assist.get("entities", []) if str(x).strip()];evidence = " ".join(str(getattr(hit, "text", "") or "") for hit in (result.get("hits") or [])).casefold()
...
def _runtime_phase_implementation(completed: dict[str, Any], final_matrix: list[Any], final_verification: dict[str, Any]) -> dict[str, Any]:
    phases=completed.get("phases") or {};plan=completed.get("phase_plan") or {};retrieval=completed.get("adaptive_retrieval") or {};two_stage=completed.get("two_stage_synthesis") or {};blocked=int(final_verification.get("blocked_claims",0));p3_status="synthesized" if two_stage.get("used") else "required_abstention" if two_stage.get("required") else "extractive_verified_fallback"
    return {
...
def _is_simple_query(query_plan: dict[str, Any]) -> bool:
    intent=str(query_plan.get("intent","")).casefold();return intent in {"factual","definition"} and not bool(query_plan.get("needs_numeric")) and not bool(query_plan.get("needs_table")) and not bool(query_plan.get("needs_figure")) and not bool(query_plan.get("needs_multi_hop"))
...
def enhance_result(system: Any, question: str, result: dict[str, Any], metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    hits=list(result.get("hits") or []);source_ids=[f"S{i+1}" for i in range(len(hits))];claims=_claim_texts(result);matrix=build_claim_evidence_matrix(claims,hits,source_ids) if claims and hits else ();hierarchy=build_evidence_hierarchy(hits);context_levels=select_context_levels(hierarchy);understanding=result.get("semantic_understanding") or {};query_plan=result.get("query_analysis") or {}
    ...
    hard_risk=bool(phase_plan.get("needs_numeric")) or str(phase_plan.get("intent","")) in {"diagnosis","management","etiology","mechanism","prognosis"};final_allowed=bool(final_verification.get("allow"))
