...
def enhance_result(system: Any, question: str, result: dict[str, Any], metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # ClaimEvidenceRecord stores the authoritative aggregate as `support`; the
    # per-span `entailment` lives inside EvidenceSpan and is not a record field.
    ...
    completed["phase_implementation"]={"phase_1_query_understanding":True,"phase_2_retrieval_precision":True,"phase_3_two_stage_generation":bool(completed.get("two_stage_synthesis",{}).get("attempted") or completed.get("extractive_stage",{}).get("supported")),"phase_4_verification":True,"phase_5_intelligence_visibility":True,"hardware_profile":"llama3.2:3b + nomic-embed-text + i5/16GB","final_claim_evidence_hard_gate":True,"final_confidence_is_calibrated":True}
