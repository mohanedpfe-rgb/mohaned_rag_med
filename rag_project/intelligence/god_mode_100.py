...
def _synthesize_from_evidence(system: Any, question: str, evidence_bundle: str) -> str | None:
    ...
    prompt = (
        ...
        "Answer ONLY from the supplied evidence. Every factual sentence MUST end with one or more "
        "existing [S#] citations. Do not invent facts, numbers, diagnoses, recommendations, causes, "
        "or qualifiers. Preserve negation and units. Return only the answer.\n\n"
...
def _diagnostic_enhance(system: Any, question: str, result: dict[str, Any], metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    """Non-authoritative enrichment. Existing answers are never discarded here."""
    ...
    try:
        ...
        grounding = _safe_dict(result.get("grounding"))
        contradiction = _safe_dict(result.get("contradiction_report"))
        ...
        enhanced.update({
            "evidence_claim_matrix": [r.to_dict() for r in matrix],
            "evidence_hierarchy": [r.to_dict() for r in hierarchy[:100]],
            ...
            "confidence": {"level": calibration.get("level", "low"), "evidence_confidence": calibration.get("calibrated", 0.0)},
            "god_mode_100": True,
            "evidence_first": bool(result.get("evidence_first", True)),
            "document_aware": bool(result.get("document_aware", True)),
            "pipeline_authority": PIPELINE_AUTHORITY,
        })
    except Exception:
        enhanced.setdefault("god_mode_100", True)
        enhanced.setdefault("evidence_first", True)
        enhanced.setdefault("pipeline_authority", PIPELINE_AUTHORITY)
    return enhanced
...
def _final_verification(answer: str, hits: list[Any], grounding: dict[str, Any], claims: list[dict[str, Any]], citations: list[Any]) -> dict[str, Any]:
    """Build the public verification object; use the canonical contract when possible."""
    try:
        contract = verify_final_answer(answer, hits)
        result = dict(contract)
        result.setdefault("checked", True)
        result.setdefault("claim_checks", claims)
        result.setdefault("citation_count", len(citations))
        result.setdefault("supported_ratio", float(grounding.get("supported_ratio", 0.0) or 0.0))
        result.setdefault("evidence_claim_matrix", [])
        return result
    except Exception:
        ratio = float(grounding.get("supported_ratio", 0.0) or 0.0)
        allow = bool(grounding.get("allow"))
        return {"checked": True, "allow": allow, "reason": grounding.get("method", "grounding_gate"), "claim_count": len(claims), "blocked_claims": 0 if allow else len(claims), "supported_ratio": ratio, "matrix_all_entailed": allow, "citation_count": len(citations), "claim_checks": claims, "evidence_claim_matrix": []}
...
def _runtime_phase_implementation(result: dict[str, Any], hits: list[Any], final_matrix_or_checks: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "phase_5_intelligence_visibility": {"status": phases.get("phase_5_intelligence_visibility", "complete"), "authority": LEGACY_TELEMETRY_AUTHORITY, "canonical_answer_authority": LEGACY_TELEMETRY_AUTHORITY, "implementation": IMPLEMENTATION_AUTHORITY},
    }
...
def enhanced_god_answer(self: Any, question: str, metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if synthesized:
        try:
            ...
            checks = list(verify_claims(synthesized, [str(getattr(h, "text", "") or "") for h in candidate_hits], [f"S{i + 1}" for i in range(len(candidate_hits))]))
            ...
            all_cited = bool(markers) and len(markers) >= max(1, len(_sentence_units(synthesized)))
            if checks and ground.get("allow") and all_cited:
                answer = synthesized
    ...
    if generation_path == "deterministic_extractive":
        ...
        try:
            from rag_project.intelligence.evidence_guard import verify_claims, grounding_decision
            checks = list(verify_claims(answer, [str(getattr(h, "text", "") or "") for h in selected], [f"S{i + 1}" for i in range(len(selected))]))
            grounding = dict(grounding_decision(checks, min_supported_ratio=0.70) if checks else {"allow": False, "supported_ratio": 0.0})
    ...
    if not bool(final_verification.get("allow", grounding.get("allow"))):
        # Exact extractive provenance is authoritative for copied source sentences.
        if generation_path == "deterministic_extractive" and grounding.get("allow"):
            final_verification["allow"] = True
            final_verification["blocked_claims"] = 0
        else:
    ...
    implementation = _runtime_phase_implementation(result, selected, final_verification)
    result["phase_implementation"] = implementation
...
def enhance_result(system: Any, question: str, result: dict[str, Any] | None = None, metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    With a preliminary result mapping this performs diagnostics/enrichment.
    Without a preliminary result it delegates to the authoritative document-aware
    answer implementation. This allows older callers to retain the public symbol
    while the canonical service still executes the real answer path.
    """
