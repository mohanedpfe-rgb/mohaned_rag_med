...
def _is_simple_query(query_plan: dict[str, Any]) -> bool:
    intent = str(query_plan.get("intent", "")).casefold()
    return intent in {"factual", "definition"} and not bool(query_plan.get("needs_numeric")) and not bool(query_plan.get("needs_table")) and not bool(query_plan.get("needs_figure")) and not bool(query_plan.get("needs_multi_hop"))
...
def enhance_result(system: Any, question: str, result: dict[str, Any], metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    budget = choose_retrieval_budget(query_tokens=len(meaningful_tokens(question)), entity_count=len(query_plan.get("entities", ()) or ()), intent=str(query_plan.get("intent", "")), confidence=float(understanding.get("confidence", 0.0) or 0.0), initial_score=float(result.get("semantic_alignment", {}).get("score", 0.0) or 0.0))
    ...
    # Simple factual/definition queries do not need generative synthesis. Temporarily
    # disabling the runtime LLM keeps all deterministic retrieval/verification phases
    # active while preventing an unnecessary slow Ollama call.
    simple = _is_simple_query(query_plan)
    saved_llm = getattr(system, "llm", None) if simple and system is not None else None
    if simple and system is not None:
        system.llm = None
    try:
    ...
    final_verification = verify_final_answer(provisional_answer, final_hits, require_entailment=bool(phase_plan.get("needs_numeric")) or str(phase_plan.get("intent", "")) in {"diagnosis", "management", "etiology", "mechanism", "prognosis"})
    ...
    entity_report = score_entity_coverage(str(completed.get("rewritten_question") or question), final_hits, planned_entities=phase_plan.get("entities") or ())
    completed["entity_coverage"] = entity_report
    hard_risk = bool(phase_plan.get("needs_numeric")) or str(phase_plan.get("intent", "")) in {"diagnosis", "management", "etiology", "mechanism", "prognosis"}
    ...
    if not final_allowed or (hard_risk and not bool(final_verification.get("matrix_all_entailed"))):
        ...
        completed["answer"] = "I could not verify a sufficiently grounded answer from the indexed evidence; unsupported or conflicting clinical details were withheld."
        completed["citations"] = []
    ...
    if not final_allowed:
        completed_calibration = calibrate_confidence(retrieval=top_score, rerank=top_score, entailment=0.0, entity_coverage=(1.0 if entity_report.get("entity_count", 0) == 0 else 0.0), source_agreement=source_agreement, contradiction=max(contradiction, 1.0), safety_conflict=max(safety_conflict, 0.0))
    ...
    completed["phase_implementation"] = _runtime_phase_implementation(completed, final_matrix, final_verification)
