    ...
    for variant in variants[: max(2, int(getattr(system.settings, "max_query_variants", 8)))]:
        try:
            hits = system.retriever.retrieve(
                _clean(variant),
                top_k=max(8, int(system.settings.top_k) * int(getattr(system.settings, "retrieval_candidate_multiplier", 5))),
                where=where,
            )
...
def _answer_with_ladder(system: Any, question: str, context: str, selected_hits: Sequence[Any], conversation_context: str) -> tuple[str, str]:
    ...
    try:
        answer = system.llm.generate(
            prompt="Return only directly supported facts from the evidence. Use [S#] markers.\n\n" + context,
...
def _god_answer(self: Any, question: str, metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not plan.normalized:
        return {"status": "LOW_QUALITY_QUERY", "answer": "Please provide a precise question.", "citations": [], "hits": [], "confidence": {"level": "none"}, "query_analysis": plan.to_dict()}
    try:
        self._ensure_embedding_dimension()
        compatibility = self.vector_store.compatibility_report(self.embedding_service.identity)
    except Exception as exc:
        return {"status": "INTERNAL_ERROR", "answer": "The retrieval index is temporarily unavailable.", "error": str(exc), "citations": [], "hits": [], "confidence": {"level": "unavailable"}, "query_analysis": plan.to_dict()}
    if str(compatibility.get("status", "")).upper() != "READY":
        return {"status": compatibility.get("status", "NOT_READY"), "answer": compatibility.get("message", "The index is not ready."), "citations": [], "hits": [], "confidence": {"level": "unavailable"}, "query_analysis": plan.to_dict()}
    try:
    ...
    if not hits:
        return {"status": "NOT_SUPPORTED", "answer": "I could not find sufficient evidence in the indexed documents to answer this question.", "citations": [], "hits": [], "confidence": {"level": "none"}, "query_analysis": plan.to_dict()}
    try:
    ...
    if alignment.get("decision") == "NOT_SUPPORTED":
        return {"status": "NOT_SUPPORTED", "answer": "The indexed evidence does not directly support this question.", "citations": [], "hits": hits[:self.settings.top_k], "confidence": {"level": "none"}, "query_analysis": plan.to_dict(), "evidence_alignment": alignment}
    ...
    evidence_conf = evidence_confidence(
        retrieval=min(1.0, max((float(h.score) for h in selected), default=0.0)),
        rerank=min(1.0, max((float(h.score) for h in selected), default=0.0)),
        entailment=sum(c.support for c in claims) / max(len(claims), 1) if claims else 0.0,
        quality=sum(min(1.0, len(enrich_text(h.text).get("keywords") or []) / 12.0) for h in selected) / max(len(selected), 1),
