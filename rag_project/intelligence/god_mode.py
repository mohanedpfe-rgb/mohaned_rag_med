...
def _safe_hits(system: Any, query: str, plan: QueryPlan, where: dict[str, Any] | None) -> list[Any]:
    ...
    for variant in queries[:8]:
        try:
            hits = system.retriever.retrieve(_clean_variant(variant), top_k=max(8, system.settings.top_k * 4), where=where)
    ...
    if not candidates:
        return []
    # Rerank a broad fused candidate set once. Existing reranker remains the semantic judge.
    try:
        ...
        reranked = candidates
    # Query-intent metadata boosts are deliberately small: retrieval evidence still dominates.
    boosted = []
    for hit in reranked:
        ...
        if plan.needs_table and ("table" in types or meta.get("table_id")):
            boost *= 1.15
        if plan.entities:
            text_low = hit.text.casefold()
            matches = sum(1 for e in plan.entities if e in text_low)
            boost *= 1.0 + min(matches, 3) * 0.04
        hit.score *= boost
        boosted.append(hit)
...
def _quality_from_hits(hits: Sequence[Any]) -> float:
    ...
    values = []
    for hit in hits:
        values.append(enrich_text(hit.text).get("keywords") and min(1.0, len(enrich_text(hit.text)["keywords"]) / 12.0) or 0.0)
...
def _god_answer(self: Any, question: str, metadata_filter: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Preserve the original system's model/index compatibility firewall.
    try:
    ...
    # Context builder handles the existing parent/neighbor/token-budget strategy.
    ...
    # Strong fail-closed path before generation.
    ...
    if pre_conf < 0.25 or (answerable < 0.15 and local < 0.10):
    ...
    ocr_penalty = 1.0 if any(str((h.metadata or {}).get("ocr_confidence", "")).strip() and float((h.metadata or {}).get("ocr_confidence")) < 0.55 for h in selected_hits if str((h.metadata or {}).get("ocr_confidence", "")).strip().replace(".", "", 1).isdigit()) else 0.0
    final_conf = evidence_confidence(retrieval=min(1.0, top), rerank=min(1.0, max(h.score for h in selected_hits)), entailment=entailment, quality=_quality_from_hits(selected_hits), contradiction=contradiction, ocr_penalty=ocr_penalty)
