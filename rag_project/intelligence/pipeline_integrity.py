...
_INTERNAL_LABELS = {
    "follow-up", "follow up", "relevant entities", "query entities", "planned entities",
    "intent", "planner", "planner confidence", "conversation context", "semantic understanding",
    "query analysis", "retrieval state", "evidence confidence", "final gate", "claim matrix",
}
...
_ABSTENTION_PREFIXES = (
    ...
    "unable to verify",
    "unsupported content was withheld",
)
...
def safe_extract_query_entities(question: str, planned_entities: Iterable[str] = ()) -> tuple[str, ...]:
    """Return only validated medical concepts, measurements and strong single-token planner concepts."""
    found: list[str] = []
    ...
    # LLM/planner entities are accepted only as atomic, non-protocol medical-looking
    # tokens. Multi-word planner prose is deliberately rejected to prevent arbitrary
    # phrases such as "relevant entities" from becoming hard requirements.
    for item in planned_entities:
        raw = _normalize(item)
        if not raw or _contains_internal_label(raw) or len(raw) > 64 or " " in raw:
            continue
        medical_like = bool(re.search(r"[-_0-9]", raw) or raw.endswith(_DRUG_SUFFIXES) or raw.endswith(_CONDITION_SUFFIXES) or len(raw) >= 7)
        if medical_like and raw not in found:
            found.append(_canonical(raw))

    cleaned: list[str] = []
    seen: set[str] = set()
    for item in found:
        normalized = _normalize(item)
        if not normalized or normalized in seen or normalized in _STOPWORDS or _contains_internal_label(normalized):
            continue
        seen.add(normalized)
...
def safe_score_entity_coverage(question: str, evidence: Sequence[Any], planned_entities: Iterable[str] = ()) -> dict[str, Any]:
    ...
    for index, hit in enumerate(evidence):
        for entity in _evidence_entities(str(getattr(hit, "text", "") or "")):
            evidence_entities.append((entity, f"S{index + 1}"))
    evidence_norms = [name for name, _ in evidence_entities]
    source_counts = Counter(source for _, source in evidence_entities)
    covered: list[str] = []; missing: list[str] = []; partial: list[str] = []; per_entity: list[dict[str, Any]] = []; matches: list[dict[str, Any]] = []
    ...
    denominator = max(1, len(query_entities)); coverage = len(covered) / denominator; partial_coverage = (len(covered) + 0.5 * len(partial)) / denominator
...
def _looks_like_followup(question: str, history: Sequence[tuple[str, str]]) -> bool:
    ...
    if re.search(r"\b(it|this|that|they|them|those|these|what about|how about|the latter|the former)\b", cleaned, re.I): return True
...
def safe_rewrite_follow_up(question: str, history: Sequence[tuple[str, str]] | None = None) -> str:
    ...
    # Keep a natural previous-turn anchor. This satisfies contextual retrieval while
    # avoiding synthetic protocol text and remains stable under repeated calls.
...
def safe_verify_final_answer(answer: str, hits: Sequence[Any], *, require_entailment: bool = False) -> dict[str, Any]:
    ...
    if is_control_message(answer):
        return {"checked": False, "allow": False, "reason": "abstention_not_claim", "claim_count": 0, "blocked_claims": 0, "supported_ratio": 0.0, "matrix_claim_count": 0, "matrix_all_entailed": False, "claim_checks": [], "evidence_claim_matrix": []}
    ...
    blocked_checks = [check for check in checks if check.status in {"UNSUPPORTED", "WEAK", "NUMERIC_MISMATCH", "CONTRADICTED"} or check.contradiction]
    ...
    elif support_ratio < 0.60: reason = "support_ratio_below_threshold"
