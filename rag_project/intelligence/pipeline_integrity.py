...
def safe_extract_query_entities(question: str, planned_entities: Iterable[str] = ()) -> tuple[str, ...]:
    ...
    for item in planned_entities:
        raw = _normalize(item)
        if not raw or len(raw) > 64 or " " in raw or ":" in raw or _contains_internal_label(raw):
            continue
    ...
    for value in found:
        ...
        if not value or value in seen or value in _STOPWORDS or _contains_internal_label(value):
            continue
        seen.add(value)
        unique.append(value)
        if len(unique) >= 32:
...
def safe_score_entity_coverage(question: str, evidence: Sequence[Any], planned_entities: Iterable[str] = ()) -> dict[str, Any]:
    ...
    query_entities = safe_extract_query_entities(question, planned_items)
    evidence_entities = []
    ...
    for entity in query_entities:
        ...
        per_entity.append({"entity": entity, "status": status, "match_score": round(best, 3), "sources": candidate_sources})
...
def safe_rewrite_follow_up(question: str, history: Sequence[tuple[str, str]] | None = None) -> str:
    ...
    anchor_question = next((str(q).strip() for q, _ in reversed(recent) if str(q or "").strip()), "")
    ...
    try:
        for entity in extract_clinical_entities(anchor_answer):
            ...
            if term and term not in context_terms and not _contains_internal_label(term):
                context_terms.append(term)
    except Exception:
        pass
    for term in _open_set_medical_terms(anchor_answer):
        if term not in context_terms and not _contains_internal_label(term):
            context_terms.append(term)
    context = " ".join(context_terms[:6])
    # Public query text never contains protocol labels. The production rewriter
    # is responsible for any internal telemetry prefix when needed.
    return re.sub(r"\s+", " ", " ".join(part for part in (anchor_question, context, cleaned) if part).strip())[:3500]
