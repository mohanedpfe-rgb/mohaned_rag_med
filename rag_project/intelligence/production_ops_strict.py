...
class MetricsService:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        by_intent: dict[str, dict[str,int]] = {}; by_path: dict[str,int] = {}; latencies: list[float] = []
        for intent, path, latency, _ in rows:
            bucket = by_intent.setdefault(intent or "unknown", {"total":0,"positive":0,"negative":0}); bucket["total"] += 1
            by_path[path or "unknown"] = by_path.get(path or "unknown",0) + 1; latencies.append(float(latency or 0.0))
        ...
        positive = sum(v for k,v in feedback_counts.items() if k in {"correct","helpful"}); negative = sum(v for k,v in feedback_counts.items() if k in {"wrong","incomplete"})
        accuracy = {k:{**v,"observed_accuracy":v["positive"]/(v["positive"]+v["negative"]) if v["positive"]+v["negative"] else None,"feedback_count":v["positive"]+v["negative"]} for k,v in by_intent.items()}
        return {"accuracy_by_type":accuracy,"latency_percentiles":{"p50":self._percentile(latencies,.50),"p95":self._percentile(latencies,.95),"p99":self._percentile(latencies,.99)},"path_usage":by_path,"feedback":feedback_counts,"error_rates":{"negative_feedback_rate":negative/(positive+negative) if positive+negative else 0.0}}
    def alerts(self, *, latency_p95_ms: float=20_000.0, failure_rate: float=0.15, accuracy_floor: float=0.95) -> list[str]:
        snap=self.snapshot(); alerts=[]
...
def two_proportion_z_test(success_a: Sequence[float], success_b: Sequence[float])->SignificanceResult:
    a=[1.0 if float(x)>0.5 else 0.0 for x in success_a]; b=[1.0 if float(x)>0.5 else 0.0 for x in success_b]; n1,n2=len(a),len(b)
    if n1<2 or n2<2: return SignificanceResult("two-proportion-z",None,None,False,n1,n2)
    p1,p2=statistics.fmean(a),statistics.fmean(b); pooled=(sum(a)+sum(b))/(n1+n2); se=math.sqrt(max(pooled*(1-pooled)*(1/n1+1/n2),1e-18)); z=(p1-p2)/se; p=2*(1-_normal_cdf(abs(z)))
...
class ABTestManager(_ABTestManager):
    def compare(self, experiment: str, *, min_samples_per_variant: int=500)->dict[str,Any]:
        ...
        summary={v:{"n":len(xs),"success_rate":statistics.fmean(x[0] for x in xs) if xs else 0.0,"latency_ms":statistics.fmean(x[1] for x in xs) if xs else 0.0,"satisfaction":statistics.fmean(x[2] for x in xs) if xs else 0.0} for v,xs in groups.items()}
        variants=sorted(groups); significance=None; candidate=None
        if len(variants)>=2:
            a,b=variants[0],variants[1]; significance=two_proportion_z_test([x[0] for x in groups[a]],[x[0] for x in groups[b]]).as_dict(); candidate=max(variants,key=lambda k:(summary[k]["success_rate"],-summary[k]["latency_ms"],summary[k]["satisfaction"]))
        ...
        return {"experiment":experiment,"variants":summary,"candidate_winner":candidate,"winner":candidate if eligible else None,"rollout_eligible":eligible,"min_samples_per_variant":min_samples_per_variant,"statistical_test":significance}
...
class VerifiedBackupManager(_BackupManager):
    ...
    def backup(self)->Path:
        destination=super().backup(); manifest={}
        for item in sorted(destination.rglob("*")):
            if item.is_file(): manifest[str(item.relative_to(destination))]=self._sha256(item)
        (destination/"SHA256SUMS.json").write_text(json.dumps(manifest,indent=2,sort_keys=True),encoding="utf-8"); return destination
    @classmethod
