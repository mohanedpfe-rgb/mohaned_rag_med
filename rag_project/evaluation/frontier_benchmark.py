...
def evaluate_case(c:BenchmarkCase,r:dict[str,Any])->BenchmarkResult:
    a=r.get('query_analysis') or {}; ents={str(x).casefold() for x in a.get('entities',())}; expected={str(x).casefold() for x in c.expected_entities}; entity_hit=bool(expected and expected&ents) if expected else bool(c.answerable and ents)
    intent_hit=not c.expected_intent or str(a.get('intent',''))==c.expected_intent
    hit_ids={str((getattr(h,'metadata',{}) or {}).get('chunk_id') or getattr(h,'doc_id','')) for h in (r.get('hits') or [])}; evidence_hit=not c.gold_evidence_ids or bool(hit_ids&set(c.gold_evidence_ids))
