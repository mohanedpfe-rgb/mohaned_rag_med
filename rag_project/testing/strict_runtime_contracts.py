...
def _with_provenance(original, phase, results):
    ...
    try:
        ...
        status = subprocess.run(["git","status","--porcelain"],cwd=ROOT,text=True,capture_output=True,timeout=10,check=True).stdout.strip(); expected=os.getenv("GITHUB_SHA","").strip(); provenance_ok=bool(sha) and not status and (not expected or sha==expected)
...
def _strict_semantic_wrapper(original):
    def wrapped(results: dict[int, PhaseResult]):
        failures=list(original(results)); extra: list[dict[str,Any]]=[]
        try:
        ...
        for number, impl in ((3,"strict_diagnostic_chain"),(4,"strict_contract_triangulation"),(5,"strict_cross_layer_invariants")):
            item=results.get(number); details=item.details if item else {}
            if item is None or details.get("authoritative_implementation") != impl or not details.get("fault_sensitivity_verified") or int(details.get("fault_probe_count") or 0) < 1:
        ...
        if p6 is None or d6.get("authoritative_implementation")!="strict_information_loss" or not d6.get("baseline_healthy_fixture_pass") or not d6.get("fault_sensitivity_verified") or int(d6.get("fault_probe_count") or 0)<2: extra.append({"phase":6,"reason":"fault-sensitive information-loss evidence incomplete"})
