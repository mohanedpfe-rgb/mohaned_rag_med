...
def strict_resource_stability(phase: Any) -> PhaseResult:
    ...
    try:
        ...
        rss_delta = payload.get("rss_delta_bytes"); fd_delta = payload.get("fd_delta"); tail_head = payload.get("rss_tail_minus_head_mean_bytes"); slope = payload.get("rss_slope_bytes_per_iteration")
        samples = int(payload.get("sample_count") or 0); iterations = int(payload.get("iterations") or 0); successful = int(payload.get("successful_ingestions") or 0)
        ...
        fd_ok = fd_delta is None or abs(int(fd_delta)) <= 2; rss_delta_ok = rss_delta is None or float(rss_delta) <= 64 * 1024 * 1024; enough_observation = samples >= 3 and iterations >= 3 and successful >= 3
        result.details = {
            "evidence_level": "real_subprocess_resource_observation",
            "evidence_level_extended": "trend_aware_subprocess_resource_observation",
            "requested_seconds": duration, "requested_mode": requested_mode, "observed_seconds": payload.get("observed_seconds"), "sample_count": samples,
            "iterations": iterations, "repetitions": iterations, "successful_ingestions": successful,
            "rss_first_bytes": payload.get("rss_first_bytes"), "rss_last_bytes": payload.get("rss_last_bytes"), "rss_peak_bytes": payload.get("rss_peak_bytes"), "rss_delta_bytes": rss_delta,
            "rss_slope_bytes_per_iteration": slope, "rss_first_quarter_mean": payload.get("rss_first_quarter_mean"), "rss_last_quarter_mean": payload.get("rss_last_quarter_mean"), "rss_tail_minus_head_mean_bytes": tail_head,
            "fd_first": payload.get("fd_first"), "fd_last": payload.get("fd_last"), "fd_delta": fd_delta, "fd_peak": payload.get("fd_peak"),
            "trend_budget_bytes_per_iteration": max_reasonable_slope, "rss_trend_ok": trend_ok, "rss_delta_ok": rss_delta_ok, "fd_leak_ok": fd_ok, "observation_depth_ok": enough_observation,
            "exit_code": proc.returncode, "workload": payload.get("workload"),
            ...
            "long_running_24h_mode_supported": True, "certification_mode": "24h_observation" if requested_mode == "24h" else "bounded_smoke",
        }
        passed = proc.returncode == 0 and enough_observation and trend_ok and rss_delta_ok and fd_ok
        result.score = 1.0 if passed else 0.0; result.status = "PASS" if passed else "FAIL"
