...
def strict_resource_stability(phase: Any) -> PhaseResult:
    ...
    try:
        ...
        requested_mode = os.getenv("DIAGNOSTIC_RESOURCE_MODE", "bounded").strip().lower(); requested_seconds = float(os.getenv("DIAGNOSTIC_RESOURCE_SECONDS", "20")); duration = max(requested_seconds, 86400.0) if requested_mode == "24h" else requested_seconds
