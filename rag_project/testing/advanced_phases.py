...
def metamorphic(phase: Any) -> PhaseResult:
    ...
    try:
        ...
        checks = {"query_whitespace_case_invariant": len(set(normalized)) == 1, "tokenization_invariant": all(tokenized[0] == tokens for tokens in tokenized[1:]), "lexical_score_invariant": max(scores) - min(scores) <= 1e-12, "chunk_content_invariant": overlap >= 0.90, "query_variants_exercised": len(queries) >= 3, "normalized_outputs_available": bool(normalized), "retrieval_path_exercised": bool(scores), "chunk_variants_exercised": bool(canonical and variant)}
