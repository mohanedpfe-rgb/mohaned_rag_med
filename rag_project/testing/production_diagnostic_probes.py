...
def phase12_stable_fingerprinting(phase: Any, results: dict[int, PhaseResult]) -> PhaseResult:
    runner = _runner()
    base = runner._hardened_fingerprinting(phase, results)
    try:
        ...
        fa = runner._hardened_fingerprinting(phase, a).details["fingerprints"][0]["fingerprint"]
        fb = runner._hardened_fingerprinting(phase, b).details["fingerprints"][0]["fingerprint"]
        fc = runner._hardened_fingerprinting(phase, c).details["fingerprints"][0]["fingerprint"]
        stable = fa == fb
        discriminates = fa != fc
...
def phase17_strict_completion(phase: Any, results: dict[int, PhaseResult]) -> PhaseResult:
    runner = _runner()
    # The authoritative base gate lives on runner and is called before this wrapper
    # only through the dedicated alias below, so no monkeypatch recursion is possible.
    ...
    if evidence_failures:
        ...
        base.status = "FAIL"; base.score = 0.0
