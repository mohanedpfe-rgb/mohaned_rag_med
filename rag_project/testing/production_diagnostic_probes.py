...
def phase13_known_causal_graph(phase: Any, results: dict[int, PhaseResult]) -> PhaseResult:
    ...
    try:
        ...
        propagation_verified = shared_failures and all(runtime_locations.values()) and {("p5f0", "p9f0"), ("p5f0", "p10f0")} <= injected_edges and "p5f0" in set(injected.details["candidate_roots"])
        base = runner._hardened_causal_graph(phase, results)
        base.details.update({"known_causal_fixture_verified": known_fixture_verified, "known_failure_injection_verified": propagation_verified, "known_fixture_expected_edges": sorted(expected), "known_fixture_observed_edges": sorted(edges), "known_failure_injection_observed_edges": sorted(injected_edges), "runtime_propagation_phases": sorted(injected_results), "runtime_failure_identities": {str(k): item.failures[0].get("exception") for k, item in injected_results.items()}, "runtime_failure_locations_verified": runtime_locations, "causal_validation_mode": "synthetic_fixture_plus_real_runtime_fault_injection_across_phases_5_9_10", "causal_root_is_injected_dependency": propagation_verified})
