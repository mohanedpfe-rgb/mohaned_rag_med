...
def _metadata_document_ids(payload: dict[str, Any]) -> list[str]:
    ...
    for value in values:
        if isinstance(value, dict):
            ...
            if document_id is not None: found.append(str(document_id))
            continue
        raw = str(value)
        try: metadata = json.loads(raw.replace("'", '"')) if raw.startswith("{") else None
...
def phase16_production_ingestion_benchmark(phase: Any) -> PhaseResult:
    ...
    try:
        ...
        temporary_root = Path(tempfile.mkdtemp(prefix="rag_phase16_production_ingestion_")); system = _ProductionIngestionProbeSystem(temporary_root)
        source_dir = temporary_root / "source"; source_dir.mkdir(parents=True, exist_ok=True)
        document_ids: dict[str, str] = {}; ingestion_rows: list[dict[str, Any]] = []
        for row in corpus:
            source = source_dir / f"{row['doc_id']}.pdf"; _write_probe_pdf(source, row["text"]); outcome = robust_ingest_file(system, source)
            if outcome.get("status") not in {"success", "skipped"}: raise RuntimeError(f"production ingestion did not reach success state: {outcome}")
            ...
            ingestion_rows.append({"source_doc_id": row["doc_id"], "document_id": document_id, "status": outcome.get("status"), "timings_ms": outcome.get("timings_ms", {}), "pages": outcome.get("page_count"), "chunks": outcome.get("chunk_count"), "embeddings": outcome.get("embedding_count"), "retrieval_mode": outcome.get("retrieval_mode")})
        ...
        for source_doc_id, document_id in document_ids.items():
            ...
            content_hash = system._hash_file(processed); validation = system.vector_store.validate_document_index(document_id, content_hash)
            ready_states += int(system.state_store.is_ready_status(state.get("status"))); indexed_chunks += int(validation.get("count") or 0)
        ...
        evidence_hits = 0; evidence_term_total = 0
        for case in gold:
            ...
            lexical = system.vector_store.search_lexical(case["question"], n_results=min(8, max(1, indexed_chunks))); query_embedding = system.embedding_service.embed_texts([case["question"]])[0]; semantic = system.vector_store.search(query_embedding, n_results=min(8, max(1, indexed_chunks)))
            ...
            retrieved_text = (_retrieved_text(lexical) + " " + _retrieved_text(semantic)).casefold()
            matched_terms = sorted(term for term in expected_terms if term in retrieved_text)
            evidence_term_total += len(expected_terms); evidence_hits += len(matched_terms)
            evidence_supported = bool(expected_terms) and set(matched_terms) == set(expected_terms)
        ...
        result.details = {
            "evidence_level": "real_pdf_extraction_to_storage_retrieval_and_evidence_grounding",
            ...
            "gold_labels_independent_of_corpus_text": True, "durable_state_verified": ready_states == len(corpus), "index_integrity_verified": indexed_chunks > 0,
            "publication_verified": True, "ingestion_rows": ingestion_rows, "results": retrieval_rows,
            "clinical_correctness_claimed": False, "clinical_correctness_not_established_reason": "benchmark validates source retrieval and evidence-term grounding, not medical truth beyond the supplied reference corpus",
        }
        ...
        if temporary_root is not None: _cleanup_store(temporary_root)
    return result
