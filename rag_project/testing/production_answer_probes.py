...
class _ProbeSystem:
    def __init__(self, root: Path) -> None:
        ...
        if self.live_ollama:
            self.llm = OllamaLLMClient(os.getenv("OLLAMA_BASE_URL", "http://127.0.0.1:11434"), os.getenv("GENERATION_MODEL", "llama3.2:3b"), timeout_seconds=60, max_output_tokens=512, circuit_threshold=2, circuit_open_seconds=15)
...
def phase10_canonical_answer_engine(phase: object) -> PhaseResult:
    ...
    try:
        ...
        require_live = os.getenv("REQUIRE_LIVE_OLLAMA", "0").strip().lower() in {"1", "true", "yes", "on"}
        live_ok = system.live_ollama and system.generation_backend == "ollama" and getattr(system.llm, "last_error", None) is None
        result.details = {
            ...
            "answer_generated": bool(answer), "generation_path": response.get("generation_path"),
            "generation_backend": system.generation_backend, "generation_backend_calls": getattr(system.llm, "calls", 1),
            "retrieval_hits": len(hits), "seeded_index_records": seeded,
            "retrieval_backend": "VectorStore + HybridRetriever + deterministic EmbeddingService test backend",
            "citations_present": bool(citations), "citation_ids_valid": citation_ids_valid,
            "verification_allow": bool(verification.get("allow")), "supported_ratio": float(verification.get("supported_ratio", 0.0) or 0.0),
            "route_intent": route.get("intent"), "canonical_engine_executed": True, "retrieval_stub_used": False,
            "live_ollama_opt_in": system.live_ollama, "live_ollama_verified": bool(live_ok), "live_ollama_required": require_live,
        ...
        required = all([result.details["answer_generated"], result.details["retrieval_hits"] > 0, result.details["citations_present"], result.details["citation_ids_valid"], result.details["verification_allow"], result.details["canonical_engine_executed"], result.details["retrieval_stub_used"] is False, (not require_live or live_ok)])
