...
def _document_scope_filter(hits: list[Any], question: str) -> list[Any]:
    ...
    for hit in hits:
        ...
        haystack = " ".join(
            [str(getattr(hit, "text", "") or ""), " ".join(str(v) for v in metadata.values())]
        ).casefold()
            ...
            try:
                ...
                if isinstance(history, list) and history:
                    ...
                    if isinstance(last, dict) and str(
                        last.get("question") or last.get("user") or ""
                    ) == str(question or ""):
                        history.pop()
            except Exception:
                pass
...
def _install_publication_audit_boundary() -> None:
    ...
    def transition(self: Any, document_id: str, new_stage: str, **values: Any) -> None:
        ...
        try:
            ...
            requested_pages = int(
                values.get("total_pages", (record or {}).get("total_pages") or 0) or 0
            )
            requested_current = int(
                values.get("current_page", (record or {}).get("current_page") or 0) or 0
            )
            content_hash = str(
                values.get("content_hash", (record or {}).get("content_hash") or "") or ""
            )
            requested_index = str(
                values.get("index_state", (record or {}).get("index_state") or "READY") or "READY"
            ).upper()
            if (
            ...
            if (
                requested_pages > 0
                and requested_current == requested_pages
                and content_hash
                and requested_index == "READY"
            ):
