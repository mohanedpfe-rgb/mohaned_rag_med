...
def validate_storage_path(project_root: Path, candidate: str | Path, label: str = "path") -> Path:
    root = Path(project_root).expanduser().resolve(); target = Path(candidate).expanduser().resolve()
    try: target.relative_to(root)
    except ValueError as exc: raise ValueError(f"{label} must remain inside the BookRAG project directory.") from exc
    return target


def _resolved_ips(host: str) -> set[str]:
    try: return {item[4][0] for item in socket.getaddrinfo(host, None, type=socket.SOCK_STREAM)}
    except OSError as exc: raise ValueError("Ollama hostname could not be resolved safely.") from exc
...
def validate_ollama_url(value: str) -> str:
    raw=str(value or "").strip().rstrip("/"); parsed=urlparse(raw)
    if parsed.scheme not in {"http","https"} or not parsed.hostname or parsed.username or parsed.password: raise ValueError("Ollama URL must be an HTTP(S) URL without embedded credentials.")
    if parsed.path not in {"","/"} or parsed.query or parsed.fragment: raise ValueError("Ollama URL must contain only scheme, host and optional port.")
    host=parsed.hostname.lower()
    try: ip=ipaddress.ip_address(host)
    except ValueError: ip=None
    if host in {"localhost","ip6-localhost"} or (ip and ip.is_loopback): return raw
    allowlist={h.strip().lower() for h in os.getenv(OLLAMA_ALLOWLIST_ENV,"").split(",") if h.strip()}
    if not allowlist or host not in allowlist: raise ValueError("Remote Ollama endpoints are disabled unless the exact hostname is in BOOKRAG_OLLAMA_ALLOWLIST.")
    addresses={str(ipaddress.ip_address(x)) for x in _resolved_ips(host)}
    if not addresses or any(_is_disallowed_ip(addr) for addr in addresses): raise ValueError("Ollama hostname resolves to a private, local, reserved, or otherwise unsafe network address.")
    return raw
...
def validate_query(value: str) -> str:
    ...
    if len(question)>MAX_QUERY_CHARS: raise ValueError(f"Question is too long; maximum is {MAX_QUERY_CHARS} characters.")
    return question


def validate_pdf_payload(name: str, content: bytes) -> None:
    safe_name=Path(name).name
    if len(safe_name)>MAX_FILENAME_CHARS: raise ValueError("PDF filename is too long.")
    if len(content)>MAX_UPLOAD_BYTES: raise ValueError(f"Uploaded file '{safe_name}' exceeds the 50 MB security limit.")
    if not content.startswith(b"%PDF-"): raise ValueError(f"Uploaded file '{safe_name}' is not a valid PDF payload.")
    try:
        import fitz
        pdf=fitz.open(stream=content,filetype="pdf")
        try: validate_pdf_page_count(pdf.page_count)
        finally: pdf.close()
    except Exception as exc: raise ValueError(f"Uploaded file '{safe_name}' failed PDF structural validation.") from exc


def validate_pdf_page_count(page_count: int) -> int:
    count=int(page_count); limit=max_pdf_pages()
    if count<0: raise ValueError("PDF page count cannot be negative.")
    if count>limit: raise ValueError(f"PDF has {count} pages; the security limit is {limit} pages.")
    return count


def register_session_upload(size_bytes:int)->None:
    size=int(size_bytes)
    if size<0: raise ValueError("Upload size cannot be negative.")
    total=int(st.session_state.get("bookrag_upload_bytes",0))
    if total+size>MAX_SESSION_UPLOAD_BYTES: raise ValueError("This session exceeded the 500 MB cumulative upload security limit.")
...
def sanitize_evidence_for_prompt(text:str)->str:
    ...
    for line in value.splitlines():
        if any(re.search(pattern,line) for pattern in patterns): lines.append("[REDACTED_UNTRUSTED_INSTRUCTION]")
...
def consume_rate_limit(bucket:str,*,limit:int,window_seconds:float)->bool:
    ...
    with _RATE_LOCK:
        ...
        _RATE_STATE[key]=(start,count+1); return True
...
def enforce_private_permissions(root:Path)->None:
    ...
    try:
        ...
        for path in root.rglob("*"):
            ...
            elif path.is_file():path.chmod(0o600)
    except OSError:pass
...
def harden_system(system):
    ...
    def guarded_ingest_directory(directory=None):
        ...
        if not acquire_ingest_slot(0.1):raise RuntimeError("Global ingestion concurrency limit reached; try again shortly.")
        try:audit_event("ingest_start",detail=str(source));return original_ingest_directory(str(source))
        finally:release_ingest_slot();audit_event("ingest_finish",detail=str(source))
    def guarded_ingest_file(pdf_path):
        candidate=validate_storage_path(root,pdf_path,"PDF path")
        if candidate.suffix.lower()!=".pdf" or not candidate.is_file():raise ValueError("Unsupported or missing PDF.")
        size=candidate.stat().st_size
        if size>MAX_UPLOAD_BYTES:raise ValueError("PDF exceeds the 50 MB security limit.")
        with candidate.open("rb") as handle:payload=handle.read(min(size,MAX_UPLOAD_BYTES+1))
    ...
    def guarded_answer(question,metadata_filter=None):
        if not consume_rate_limit("answer",limit=30,window_seconds=60):raise RuntimeError("Query rate limit exceeded; wait before sending more questions.")
        validate_query(question)
        if not acquire_answer_slot(0.1):raise RuntimeError("Global answer concurrency limit reached; try again shortly.")
        try:return postprocess_medical_output(original_answer(question,metadata_filter))
