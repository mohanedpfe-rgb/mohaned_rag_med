...
def require_auth() -> bool:
    """Authenticate without forcing an extra Streamlit rerun after a valid login."""
    configured = os.getenv(AUTH_ENV, "").strip()
    accepted_passwords = {DEFAULT_LOCAL_PASSWORD}
    if configured:
        accepted_passwords.add(configured)
    if not any(len(value) >= 12 for value in accepted_passwords):
        st.error("BookRAG is locked because no configured password meets the 12-character minimum.")
        st.stop()
    ...
    st.caption("Enter your local BookRAG password to open the workspace.")
    ...
    if st.button("Unlock BookRAG", type="primary", key="bookrag_unlock"):
        unlocked = any(secrets.compare_digest(attempt, expected) for expected in accepted_passwords if len(expected) >= 12)
        if unlocked:
...
def postprocess_medical_output(result: dict) -> dict:
    ...
    answer = str(result.get("answer") or ""); citations = result.get("citations") or []
