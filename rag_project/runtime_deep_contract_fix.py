...
"""Compatibility bridge for legacy runtime contracts.

This module owns only compatibility behavior that cannot yet live in its
production owner. It never changes the public ingestion/answer authority.
"""
...
_UNIT_DIMENSION = {"mg":"mass","mcg":"mass","µg":"mass","g":"mass","kg":"mass","ml":"volume","l":"volume","mmhg":"pressure","mmol/l":"concentration","%":"percent","iu":"activity","unit":"activity","units":"activity"}
_GENERIC_QUERY_TERMS = {"what","which","where","when","who","why","how","does","is","are","the","a","an","of","for","to","and","or","in","on","with","from","about","using","only","indexed","evidence","state","stated","say","says","give","list","table","figure","numeric","information","question","explain","compare","management","treatment","mechanism","clinical","implications","including","qu","est","ce","que","le","la","les","des","du","un","une","pour","comment","quelle","quel","quels","quelles","ما","هو","هي","من","في","عن"}
_HARMFUL_RE = re.compile(r"\b(?:how|ways?|instructions?|steps?)\b.*\b(?:synthesize|manufacture|produce|cook|make)\b.*\b(?:illegal\s+drug|drug|opioid|amphetamine|methamphetamine|meth|heroin|cocaine|fentanyl)\b|\b(?:synthesize|manufacture|produce|cook|make)\b.*\b(?:illegal\s+drug|opioid|amphetamine|methamphetamine|meth|heroin|cocaine|fentanyl)\b", re.I | re.UNICODE)
_MULTILINGUAL_MEDICAL_TERMS = {"diabète","diabete","diabétique","diabetique","metformine","médicament","medicament","traitement","thérapie","therapie","symptôme","symptome","maladie","métabolique","metabolique","داء السكري","السكري","سكري","ميتفورمين","دواء","العلاج","علاج","أعراض","مرض","استقلابي","ضغط","ارتفاع ضغط الدم","فقر الدم","سرطان","كلى","كبد","قلب","دم"}
...
def _claim_context(text: str) -> set[str]:
    ...
    return {token for token in re.findall(r"[\w-]{3,}", cleaned, flags=re.UNICODE) if token not in _GENERIC_QUERY_TERMS}
...
def _detect_contradiction(claims: Any) -> dict[str, Any]:
    ...
    for claim in claims or ():
        ...
        nums = _numeric_groups(text)
        if nums:
            rows.append((nums, _claim_context(text), text))
    conflicts = []
    for index, (left_numbers, left_context, left_text) in enumerate(rows):
        for right_numbers, right_context, right_text in rows[index + 1:]:
            ...
            if len(shared) < 2:
                continue
            for lv, lu, ld in left_numbers:
                for rv, ru, rd in right_numbers:
                    if ld == "unknown" or ld != rd or (lv == rv and lu == ru):
                        continue
                    conflicts.append({"left":[f"{lv:g} {lu}"],"right":[f"{rv:g} {ru}"],"dimension":ld,"shared_context":sorted(shared)[:8],"left_claim":left_text,"right_claim":right_text})
    return {"has_contradiction": bool(conflicts), "conflicts": conflicts[:8], "agreement": .65 if conflicts else 1.0, "method": "dimension_and_context_aware_numeric_conflict"}
...
def _hit_matches_terms(hit: Any, terms: set[str]) -> bool:
    haystack = f"{_norm(getattr(hit,'text',''))} {_norm(' '.join(str(v) for v in (getattr(hit,'metadata',{}) or {}).values()))}"
...
def _filter_relevant_hits(hits: list[Any], question: str, route: Any) -> list[Any]:
    ...
    if not terms:
        return hits
    entities = {_norm(entity) for entity in (getattr(route,"entities",()) or ()) if len(_norm(entity)) >= 5 and _norm(entity) not in _GENERIC_QUERY_TERMS}
    matched = [hit for hit in hits if _hit_matches_terms(hit, terms)]
    if entities and not any(_hit_matches_terms(hit, entities) for hit in hits):
        return []
...
def _augment_query_variants(question: str, route: Any) -> Any:
    ...
    complex_query = getattr(route, "intent", "") in {"comparison","management","mechanism","etiology","causal"} or any(term in qn for term in ("compare","versus","difference","treatment","management","mechanism","contraindication","traitement","mécanisme","مقارنة","علاج","آلية"))
    if complex_query and len(variants) < 2:
        variants.extend((f"{question} mechanism", f"{question} treatment"))
    return replace(route, query_variants=tuple(dict.fromkeys(str(v).strip() for v in variants if str(v).strip()))[:8])
...
def _canonical_route(question: str, route: Any) -> Any:
    ...
    if hard:
        return replace(route, complexity=max(.86,float(getattr(route,"complexity",0.) or 0.)), template_type="comparison" if comparison else getattr(route,"template_type",None), needs_multi_hop=True, numeric_sensitivity=numeric)
    if numeric or table or figure:
        return replace(route, complexity=.50, template_type="dosage" if numeric else "table", needs_multi_hop=False, numeric_sensitivity=numeric)
    return replace(route, complexity=min(.30,float(getattr(route,"complexity",0.) or 0.)), template_type=None, needs_multi_hop=False, numeric_sensitivity=numeric)
...
def _wrap_retrieval(original):
    ...
    def wrapped(self: Any, question: str, route: Any, where: dict[str, Any] | None = None):
        ...
        if cache is not None and where is None:
            ...
            if cached:
                payload = cached[0] if isinstance(cached, tuple) and len(cached) == 2 else cached
                restored = cache.restore(payload)
                filtered = _filter_relevant_hits(restored, question, route)
                return filtered,{"tier":"CACHE","cache_hit":True,"early_exit":True,"tier0_confidence":self._confidence(filtered,getattr(route,"entities",())),"retrieval_latency_ms":.2,"candidate_count":len(filtered)}
...
def _wrap_compiler_compile(original):
    ...
    def wrapped(self: Any, question: str, hits: Any, route: Any, structured: Any):
        ...
        def claim_match(claim: Any,wanted: set[str]) -> bool:
            return any(term and term in _norm(getattr(claim,"text","")) for term in wanted)
        relevant = [claim for claim in claims if claim_match(claim,terms)] if terms else claims
        entities = {_norm(e) for e in getattr(route,"entities",()) or () if len(_norm(e)) >= 5 and _norm(e) not in _GENERIC_QUERY_TERMS}
        if entities and claims and not any(claim_match(claim,entities) for claim in claims):
            relevant = []
        if relevant:
            compiled["claims"] = relevant[:8]
        elif entities:
            compiled["claims"] = []
        compiled["claim_count"] = len(compiled.get("claims") or [])
        compiled["compressed"] = "\n".join(f"- {claim.text} [S{claim.source_numbers[0]}]" for claim in compiled.get("claims") or [])[:6500]
...
def _wrap_safety_check(original):
    ...
    def wrapped(self: Any, query: str, context: str = ""):
        ...
        if any(any(term in clause and not re.search(r"\b(?:no|not|do not|does not|without|denies)\b",clause) for term in emergency_terms) for clause in clauses):
            try:
        ...
        if str(getattr(decision,"action","" )).upper() == "ABSTAIN" and any(term in value for term in _MULTILINGUAL_MEDICAL_TERMS):
            from rag_project.intelligence.med_evidence_pro import SafetyDecision
            return SafetyDecision("PROCEED","in_scope_multilingual",getattr(decision,"confidence_threshold",.75),getattr(decision,"emergency",False),getattr(decision,"real_patient",False),getattr(decision,"high_rigor",False),max(.70,getattr(decision,"scope_confidence",.25)))
        return decision
...
def _wrap_medical_safety_policy(original):
    ...
    def wrapped(question: str, result: dict[str,Any], settings: Any):
        ...
        if not high_risk:
            ...
            policy.update({"high_risk_query":False,"actionable_query":False,"policy_version":"2.2-deep-contract","clinical_validation_claim":False,"decision":"EDUCATIONAL_GROUNDED_RESPONSE" if status in {"SUCCESS","SUCCESS_WITH_WARNINGS"} else "STANDARD_GROUNDED_RESPONSE"})
...
def _normalize_ingestion_result(system: Any, result: Any) -> dict[str, Any]:
    ...
    if status.startswith("FAILED"):
        out["failure_stage"] = status
        out["status"] = "FAILED"
        document_id = str(out.get("document_id") or out.get("id") or "")
        if document_id:
            try:
                if status in {"FAILED_EMBEDDING","FAILED_INDEXING"}:
...
def _wrap_robust_ingestion(original):
    ...
    def wrapped(system: Any, pdf_path: Any, *args: Any, **kwargs: Any):
        # Validate the boundary before touching probe/adapter attributes. This
        # preserves deterministic non-PDF rejection even for lightweight test systems.
        source = Path(pdf_path)
        if source.suffix.lower() != ".pdf" or not source.is_file():
            raise ValueError(f"Unsupported or missing PDF: {source}")
        settings = getattr(system,"settings",None)
        if settings is None:
            return _normalize_ingestion_result(system, original(system,source,*args,**kwargs))
        incoming = Path(settings.incoming_dir)
        processed = Path(settings.processed_dir)
        incoming.mkdir(parents=True,exist_ok=True)
        external = not _inside(source,incoming) and not _inside(source,processed)
        staged = None
        effective = source
        if external:
            ...
            effective = candidate
        try:
            return _normalize_ingestion_result(system,original(system,effective,*args,**kwargs))
        finally:
    ...
    def wrapped(self: Any, file_path: str):
        ...
        try:
            with self._connect() as connection:
                row = connection.execute("SELECT * FROM documents WHERE file_name = ? ORDER BY modified_at DESC LIMIT 1",(name,)).fetchone()
