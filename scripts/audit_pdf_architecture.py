...
def main() -> int:
    ...
    checks: list[tuple[str, bool]] = [
        ...
        check("12 runtime owns hierarchy anchors", "structure_anchor_runtime" in Path("rag_project/runtime.py").read_text(encoding="utf-8")),
    ]
    ...
    checks.extend(
        [
            check("13 global chapter survives page boundary", first.chapter_id == second.chapter_id),
            check("14 global section survives page boundary", first.section_id == second.section_id),
            check("15 global parent survives page boundary", first.parent_id == second.parent_id),
            check("16 chapter path exists", bool(first.hierarchy_path)),
            check("17 section id is stable", bool(first.section_id)),
        ]
    )
    ...
    checks.extend(
        [
            check("18 canonical chunks exist", bool(chunks)),
            check("19 canonical representation type", chunks and chunks[0].representation_type == "canonical"),
            check("20 canonical parent id", bool(metadata.get("parent_id"))),
            check("21 canonical section id", bool(metadata.get("section_id"))),
            check("22 canonical chapter id", bool(metadata.get("chapter_id"))),
            check("23 canonical hierarchy path", bool(metadata.get("hierarchy_path"))),
            check("24 canonical quality", metadata.get("quality_score") is not None),
        ]
    )

    with tempfile.TemporaryDirectory(prefix="rag-audit-") as directory:
        store = VectorStore(Path(directory) / "vector_db")
        checks.extend(
            [
                check("25 vector store creates", store is not None),
                check("26 structure collection hook", hasattr(store, "_deep_structure_collection")),
                check("27 vector validation hook", hasattr(store, "validate_document_index")),
                check("28 lexical storage present", Path(store.lexical_database).exists()),
            ]
        )
