...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--duration", type=float, default=5.0); args = parser.parse_args()
    try:
    ...
    started = time.monotonic(); deadline = started + max(5.0, args.duration); samples: list[int] = []; fds: list[int] = []; iterations = 0; successes = 0; failures: list[str] = []
    while time.monotonic() < deadline:
        ...
        try:
            ...
            else: failures.append(str(outcome.get("status") or "unknown_failure"))
