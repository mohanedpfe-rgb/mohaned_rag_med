...
def _install_resource_publication_guard(system: object) -> None:
    ...
    def transition(document_id: str, new_stage: str, **values: object) -> object:
        if str(new_stage).upper() in {"READY", "COMPLETED"}:
            values.setdefault("index_state", "READY")
        return original(document_id, new_stage, **values)

    transition._resource_ready_guard = True
    state_store.transition_document_state = transition
...
def main() -> int:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            _install_resource_publication_guard(system)
    ...
    payload = {
        ...
        "rss_tail_minus_head_mean_bytes": (
            _quarter_mean(samples, False) - _quarter_mean(samples, True)
