...
2. SQLite is the durable source for ingestion state; Chroma and lexical data are published with explicit readiness state and recoverable cleanup. This is a visibility transaction, not a false claim of cross-store ACID.
...
8. OCR is optional and disabled by default on the i5/16GB profile. When enabled, expensive rendering/inference is isolated behind hard time limits and bounded render sizes.
9. Retrieval is multilingual-aware for English, French, and Arabic, and generation is grounded in retrieved evidence with refusal on insufficient alignment.
10. Every production entry point must pass through the composition/runtime boundary so safety and recovery policies are installed before services are constructed.
