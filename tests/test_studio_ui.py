...
def test_active_app_uses_canva_exact_runtime() -> None:
    ...
    for marker in (
        "BookRAG Studio", "Choose PDF files", "Vector chunks", "Settings",
        "Ask Your Documents", "System Health", "Inspector", "Start all chunks",
        "Recreate runtime", "Clear all PDF data",
    ):
...
def test_real_rag_actions_are_wired() -> None:
    ...
    for marker in (
        ...
        "system.clear_pdf_data", "system.verify_index", "st.rerun",
        "get_system.clear", "apply_settings_in_place",
    ):
...
def test_upload_deduplication_and_pdf_validation_are_real() -> None:
    source = _source("rag_project/app/canva_exact_ui.py")
    for marker in (
        "hashlib.sha256", 'if digest in hashes', 'content.startswith(b"%PDF-")',
        "Path(name).name", "incoming.mkdir", "incoming / f",
    ):
        assert marker in source
...
def test_navigation_and_functional_pages_exist() -> None:
    source = _source("rag_project/app/canva_exact_ui.py")
    for marker in (
        '"Overview"', '"Documents"', '"Index them"', '"Ingestion"',
        '"Inspector"', '"Settings"', '"Chat"', '"Health"', '"Background"',
        'st.session_state["studio_nav"]', "def sidebar", "def overview", "def chat",
        "def health", "def ingestion", "def background", "def inspector", "def settings",
    ):
        assert marker in source
...
def test_health_ingestion_inspector_and_settings_have_guards() -> None:
    source = _source("rag_project/app/canva_exact_ui.py")
    for marker in (
        "ollama_health", "Configured Ollama endpoint is unavailable.",
...
def test_canva_geometry_contract_is_explicit() -> None:
    ...
    exact = _source("rag_project/app/canva_exact_ui.py")
    for marker in (
        "left:91px", "top:112px", "width:1738px", "height:855px",
...
def test_visual_system_matches_canva_style_language() -> None:
    source = _source("rag_project/app/canva_exact_ui.py")
    for marker in (
        "backdrop-filter", "index-metric", "settings-panel", "query-panel",
        "health-panel", "inspector-panel", "status", "linear-gradient",
        "--surface", "--accent", "@media", "glass-note",
    ):
        assert marker in source
