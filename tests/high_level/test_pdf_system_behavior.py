...
def test_mixed_book_preserves_hierarchy_when_table_and_figure_share_a_page() -> None:
    ...
    assert len({chunk.metadata.get("parent_id") for chunk in selected}) == 1
    assert len({chunk.metadata.get("section_id") for chunk in selected}) == 1
...
def test_two_chapters_do_not_share_structure_identity() -> None:
...
def test_nested_sections_produce_distinct_stable_section_ids() -> None:
    tracker = DocumentStructureTracker("book")
...
def test_heading_change_is_reflected_in_later_chunks_only() -> None:
    pages = [
        _page(1, "Chapter 6 Renal System\n6.1 Filtration\nFiltration text"),
        _page(2, "6.2 Reabsorption\nReabsorption text"),
    ]
...
def test_table_content_is_not_duplicated_into_canonical_prose() -> None:
    table = "Drug | Dose\nAmoxicillin | 500 mg"
    page = _page(
        6,
        f"Chapter 8 Antibiotics\n8.1 Dosing\nUse according to clinical indication.\n[TABLE]\n{table}",
        tables=[table],
    )
    chunks = SemanticChunker(280, 20).chunk_pages([page])
...
def test_multiple_tables_on_same_page_get_distinct_ids() -> None:
    page = _page(
        7,
...
def test_multiple_figures_on_same_page_get_distinct_ids() -> None:
    page = _page(
        9,
        "Chapter 11 Neurology\n11.1 Brain Imaging\nFigures below.",
        captions=["Figure 11.1. MRI anatomy", "Figure 11.2. Functional map"],
    )
    chunks = SemanticChunker(280, 20).chunk_pages([page])
...
def test_low_quality_page_preserves_quality_score_and_ocr_route() -> None:
    page = _page(
        13,
...
def test_page_order_does_not_break_structure_continuity() -> None:
    tracker = DocumentStructureTracker("book")
...
def test_figure_caption_keeps_same_structure_as_surrounding_section() -> None:
    page = _page(
        16,
        "Chapter 15 Dermatology\n15.2 Lesions\nLesion morphology.\nFigure follows.",
        captions=["Figure 15.2. Common lesion morphology"],
    )
    chunks = SemanticChunker(280, 20).chunk_pages([page])
    canonical = [c for c in chunks if c.representation_type == "canonical"]
    figures = [c for c in chunks if c.representation_type == "figure_caption"]
...
def test_chunk_metadata_is_sufficient_for_page_level_traceability() -> None:
    page = _page(
        17,
...
def test_empty_page_does_not_create_fake_semantic_content() -> None:
    page = _page(18, "   \n\n")
    chunks = SemanticChunker(280, 20).chunk_pages([page])
