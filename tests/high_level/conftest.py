...
def write_minimal_pdf(path: Path, pages: list[str]) -> Path:
    """Create a small standards-compliant PDF without a binary fixture dependency."""
    objects: list[bytes] = []
    page_refs: list[int] = []
    font_obj = 3
    next_obj = 4
    for page_text in pages:
        ...
        content_obj = next_obj; next_obj += 1
        page_obj = next_obj; next_obj += 1
        objects.extend([
            f"{content_obj} 0 obj\n<< /Length {len(stream)} >>\nstream\n".encode() + stream + b"\nendstream\nendobj\n",
...
@pytest.fixture
def clean_system(settings_i5: Settings):
    try:
        return create_rag_system(settings_i5)
