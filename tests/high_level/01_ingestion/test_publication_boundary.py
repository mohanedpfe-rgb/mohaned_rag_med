...
@pytest.mark.high_level
def test_ingestion__failed_publication_cannot_leave_a_ready_index_for_the_failed_version(clean_system, tmp_path, monkeypatch):
    ...
    assert str(result.get("status") or "").upper() not in {"READY", "COMPLETED", "SUCCESS"}
    document_id = str(result.get("document_id") or "")
    if document_id:
